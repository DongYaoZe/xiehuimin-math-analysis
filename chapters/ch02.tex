...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 判断正确与否：若 $\{a_n\}$ 收敛，则有 $\displaystyle\lim_{n\to\infty}(a_{n+1}-a_n)=0$ 和 $\displaystyle\lim_{n\to\infty}\frac{a_{n+1}}{a_n}=1$。
  \item 设收敛数列 $\{a_n\}$ 的每一项都是整数，问：该数列有什么特殊性质？
  \item 问：收敛数列是否一定是单调数列？无穷小量是否一定是单调数列？
  \item 问：一个很小很小的量，例如取 $1\,\mathrm{m}$ 为单位长度时，几个纳米大小的量是否是无穷小量？
  \item 问：正无穷大数列是否一定单调增加？无界数列是否一定是无穷大量？
  \item 问：如果数列 $\{a_n\}$ 收敛于 $a$，那么绝对值 $\abs{a_n-a}$ 是否随着 $n$ 的增加而单调减少趋于 $0$？
  \item 判断正确与否：非负数列的极限是非负数，正数列的极限是正数。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \end{enumerate}
  \item 设 $a_n\ge0,\ n\in\NN_+$，数列 $\{a_n\}$ 收敛于 $a$，证明 $\displaystyle\lim_{n\to\infty}\sqrt{a_n}=\sqrt a$。
  \item 若 $\displaystyle\lim_{n\to\infty}a_n=a$，证明 $\displaystyle\lim_{n\to\infty}\abs{a_n}=\abs a$。反之如何？
  \item 下面一组题在本章的许多极限计算中有用（并与第五章的连续性概念有关）：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 设 $a>1$，证明 $\displaystyle\lim_{n\to\infty}\frac{\log_a n}{n}=0$。
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $\{a_n\}$ 收敛而 $\{b_n\}$ 发散，问：数列 $\{a_n+b_n\}$ 和 $\{a_nb_n\}$ 的敛散性如何？
  \item 设 $\{a_n\}$ 和 $\{b_n\}$ 都发散，问：数列 $\{a_n+b_n\}$ 和 $\{a_nb_n\}$ 的敛散性如何？
  \item 设 $a_n\le b_n\le c_n,\ n\in\NN_+$，已知 $\displaystyle\lim_{n\to\infty}(c_n-a_n)=0$，问：数列 $\{b_n\}$ 是否收敛？
  \item 找出下列运算中的错误：
  \[
  ...
  \item 设已知 $\{a_n\}$ 收敛于 $a$，又有 $b\le a\le c$，问：是否存在 $N$，使得当 $n>N$ 时成立 $b\le a_n\le c$？
  \item 设已知 $\displaystyle\lim_{n\to\infty}a_n=0$，问：是否有 $\displaystyle\lim_{n\to\infty}(a_1a_2\cdots a_n)=0$？又问：反之如何？
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 设 $s_n=a+3a^2+\cdots+(2n-1)a^n$，$\abs a<1$，求 $\{s_n\}$ 的极限。

  （试计算 $s_n-as_n$。）
  \item 设正数列 $\{x_n\}$ 收敛，极限大于 0，证明：这个数列有正下界，但在数列中不一定有最小数。
  \item 证明：若有 $\displaystyle\lim_{n\to\infty}a_n=+\infty$，则在数列 $\{a_n\}$ 中一定有最小数。
...
\begin{enumerate}[label=\arabic*.]
  \item 证明：若 $\{x_n\}$ 单调，则 $\{\abs{x_n}\}$ 至少从某项开始后单调。又问：反之如何？
  \item 设 $\{a_n\}$ 单调增加，$\{b_n\}$ 单调减少，且有 $\displaystyle\lim_{n\to\infty}(a_n-b_n)=0$。证明：数列 $\{a_n\}$ 和 $\{b_n\}$ 都收敛，且极限相等。
  \item 按照极限的定义证明：单调增加有上界的数列的极限不小于数列的任何一项，单调减少有下界的数列的极限不大于数列的任何一项。
  \item 设 $x_n=\dfrac23\cdot\dfrac35\cdots\dfrac{n+1}{2n+1}$，$n\in\NN_+$，求数列 $\{x_n\}$ 的极限。
  \item 设 $a_n=\dfrac{10}{1}\cdot\dfrac{11}{3}\cdots\dfrac{n+9}{2n-1}$，$n\in\NN_+$，求数列 $\{a_n\}$ 的极限。
  \item 在例题 2.2.6 的基础上证明：当 $p>1$ 时，数列 $\{S_n\}$ 收敛，其中
  \[
    S_n=1+\frac1{2^p}+\frac1{3^p}+\cdots+\frac1{n^p},\qquad n\in\NN_+.
  \]
  \item 设 $0<x_0<\dfrac\pi2$，$x_n=\sin x_{n-1}$，$n\in\NN_+$，证明：$\{x_n\}$ 收敛，并求其极限。

  （参见 §2.6 和例题 8.1.10。）
  \item 设 $a_n=\left[\dfrac{(2n-1)!!}{(2n)!!}\right]^2$，$n\in\NN_+$，证明：$\{a_n\}$ 收敛于 0。
  ...
  \item 设 $a_n=\left[\dfrac{(2n)!!}{(2n-1)!!}\right]^2\cdot\dfrac1{2n+1}$，$n\in\NN_+$，证明：$\{a_n\}$ 收敛。
  ...
  \item 对每个正整数 $n$，用 $x_n$ 表示方程 $x+x^2+\cdots+x^n=1$ 在闭区间 $[0,1]$ 中的根。求 $\displaystyle\lim_{n\to\infty}x_n$。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $\displaystyle\lim_{n\to\infty}x_n=+\infty$，证明：$\displaystyle\lim_{n\to\infty}\frac{x_1+x_2+\cdots+x_n}{n}=+\infty$。
  \item 设 $\{x_n\}$ 单调增加，$\displaystyle\lim_{n\to\infty}\frac{x_1+x_2+\cdots+x_n}{n}=a$，证明：$\{x_n\}$ 收敛于 $a$。
  \item 设 $\{a_{2k-1}\}$ 收敛于 $a$，$\{a_{2k}\}$ 收敛于 $b$，且 $a\ne b$，求 $\displaystyle\lim_{n\to\infty}\frac{a_1+a_2+\cdots+a_n}{n}$。

  （注意：虽然数列 $\{a_n\}$ 发散，但前 $n$ 项的算术平均值所成的数列仍可以有极限。一个典型例子就是 $\{(-1)^n\}$。）
  \item 若 $\displaystyle\lim_{n\to\infty}(a_n-a_{n-1})=d$，证明：$\displaystyle\lim_{n\to\infty}\frac{a_n}{n}=d$。

  （本题可以说是 Cauchy 命题的另一种形式，也很有用。）
  \item 设 $\{a_n\}$ 为正数列，且收敛于 $A$，证明：$\displaystyle\lim_{n\to\infty}(a_1a_2\cdots a_n)^{1/n}=A$。

  （本题与 Cauchy 命题的关系是明显的。）
  \item 设 $\{a_n\}$ 为正数列，且存在极限 $\displaystyle\lim_{n\to\infty}\frac{a_{n+1}}{a_n}=l$，证明 $\displaystyle\lim_{n\to\infty}\sqrt[n]{a_n}=l$。

  （本题对类型为 $\{\sqrt[n]{a_n}\}$ 的极限问题很有用，可以说是例题 2.1.2 的一个发展。这个结果在无穷级数的研究中也很重要。）
  \item 设 $\displaystyle\lim_{n\to\infty}(x_n-x_{n-2})=0$，证明 $\displaystyle\lim_{n\to\infty}\frac{x_n}{n}=0$。
  \item 设 $\displaystyle\lim_{n\to\infty}(x_n-x_{n-2})=0$，证明：$\displaystyle\lim_{n\to\infty}\frac{x_n-x_{n-1}}n=0$。

  （本题是 1970 年的 Putnam 竞赛题，若没有题 7 的铺垫该如何做？）
  \item 设数列 $\{a_n\}$ 满足条件 $0<a_1<1$ 和 $a_{n+1}=a_n(1-a_n)$（$n\ge1$），证明：$\displaystyle\lim_{n\to\infty}na_n=1$。
  \item 若 $\displaystyle\lim_{n\to\infty}a_n=\alpha$，$\displaystyle\lim_{n\to\infty}b_n=\beta$，证明：
  \[
    \lim_{n\to\infty}\frac{a_1b_n+a_2b_{n-1}+\cdots+a_nb_1}{n}=\alpha\beta.
  \]
\end{enumerate}

\section{自然对数的底 $\ee$ 和 Euler 常数 $\gamma$}

在数学分析中数 $\ee$ 是通过数列极限而引进的一个常数，近似值为 $\ee\approx2.718\,28$。数 $\ee$ 在数学以及一般科学中的重要性决不亚于圆周率 $\pi$。

\subsection{与数 $\ee$ 有关的两个问题}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 设 $k\in\NN_+$，证明：$\displaystyle\frac{k}{n+k}<\ln\left(1+\frac kn\right)<\frac kn$。
  \item 求 $\displaystyle\lim_{n\to\infty}\left(1+\frac1{n^2}\right)\left(1+\frac2{n^2}\right)\cdots\left(1+\frac n{n^2}\right)$。
  \item 设 $\{p_n\}$ 是正数列，且 $p_n\to+\infty$，计算 $\displaystyle\lim_{n\to\infty}\left(1+\frac1{p_n}\right)^{p_n}$。
  \item 求 $\displaystyle\lim_{n\to\infty}\frac{n!\,2^n}{n^n}$。
  \item 求极限 $\displaystyle\lim_{n\to\infty}\frac{\ln n}{1+\frac12+\cdots+\frac1n}$。
  \item 证明：
  \[
    \left(\frac{n+1}{\ee}\right)^n<n!<\ee\left(\frac{n+1}{\ee}\right)^{n+1}.
  \]
  （由此又可得到 $\displaystyle\lim_{n\to\infty}\frac n{\sqrt[n]{n!}}=\ee$。）
  \item 设 $S_n=1+2^2+3^3+\cdots+n^n$，$n\in\NN_+$。证明：对 $n\ge2$，成立不等式
  \[
    n^n\left[1+\frac1{4(n-1)}\right]\le S_n
    <n^n\left[1+\frac2{\ee(n-1)}\right].
  \]
  \item 设有 $a_1=1$，$a_n=n(a_{n-1}+1)$，$n=2,3,\cdots$，又设
  \[
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 设 $A>0$，$0<b_0<A^{-1}$，$b_{n+1}=b_n(2-Ab_n)$，$n\in\NN_+$。证明：$\displaystyle\lim_{n\to\infty}b_n=A^{-1}$。
  \item 设参数 $b>4$，$x_1=\dfrac12$，$x_{n+1}=bx_n(1-x_n)$，$n\in\NN_+$。证明：$\{x_n\}$ 发散。
  \item 设 $x_1=b$，$x_{n+1}=\dfrac12(x_n^2+1)$。问：$b$ 取何值时数列 $\{x_n\}$ 收敛，并求其极限。
  \item 设 $x_0=a$，$x_n=1+bx_{n-1}$，$n\in\NN_+$。试求出使该数列收敛的 $a,b$ 的所有值。

  （本题为线性迭代，解法很多。）
  \item （对于线性迭代的全面讨论）设给定初始值 $x_1$，然后用线性函数 $f(x)=ax+b$ 迭代生成数列 $\{x_n\}$，即 $x_{n+1}=ax_n+b$。试回答以下问题：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \end{enumerate}
  \item 设 $\{x_n\}$ 为正数列，且满足 $x_{n+1}+\dfrac1{x_n}<2$，$n\in\NN_+$。证明 $\{x_n\}$ 收敛，并求其极限。
  \item 设 $A>0$，$x_1>0$，$x_{n+1}=\dfrac12\left(x_n+\dfrac A{x_n}\right)$，$n\in\NN_+$，证明：$\displaystyle\lim_{n\to\infty}x_n=\sqrt A$。
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $\{a_{2k-1}\}$，$\{a_{2k}\}$ 和 $\{a_{3k}\}$ 都收敛，证明：$\{a_n\}$ 收敛。
  \item 设 $\{a_n\}$ 有界，且满足条件 $a_n\le a_{n+2}$，$a_n\le a_{n+3}$，$n\in\NN_+$，证明：$\{a_n\}$ 收敛。
  \item 设 $\{a_n+a_{n+1}\}$ 和 $\{a_n+a_{n+2}\}$ 都收敛，证明：$\{a_n\}$ 收敛。
  \item 设数列 $\{a_n\}$ 收敛于 $0$，又存在极限 $\displaystyle\lim_{n\to\infty}\abs{\frac{a_{n+1}}{a_n}}=a$。证明：$a\le1$。
  \item 设 $a_n=\displaystyle\sum_{k=1}^n\left(\sqrt{1+\frac{k}{n^2}}-1\right)$，$n\in\NN_+$，计算 $\displaystyle\lim_{n\to\infty}a_n$。
  \item 用 $p(n)$ 表示能整除 $n$ 的素数的个数，证明：$\displaystyle\lim_{n\to\infty}\frac{p(n)}n=0$。
  \item 设 $a_0,a_1,\cdots,a_p$ 是 $p+1$ 个给定的数，且满足条件 $a_0+a_1+\cdots+a_p=0$。求
  \[
    \lim_{n\to\infty}\left(a_0\sqrt n+a_1\sqrt{n+1}+\cdots+a_p\sqrt{n+p}\right).
  \]
  \item 证明：当 $0<k<1$ 时，$\displaystyle\lim_{n\to\infty}[(1+n)^k-n^k]=0$。
  \item
  \begin{enumerate}[label=(\arabic*)]
  ...
  \end{enumerate}
  \item 证明：$\left(\dfrac n3\right)^n<n!<\left(\dfrac n2\right)^n$，其中右边的不等式当 $n\ge6$ 时成立。
  \item 证明：$\left(\dfrac n\ee\right)^n<n!<\ee\left(\dfrac n2\right)^n$。
  ...
  \end{enumerate}
  \item 设 $a_n=1+\dfrac1{\sqrt2}+\dfrac1{\sqrt3}+\cdots+\dfrac1{\sqrt n}-2\sqrt n$，$n\in\NN_+$，证明：$\{a_n\}$ 收敛。
  \item 设已知存在极限 $\displaystyle\lim_{n\to\infty}\frac{a_1+a_2+\cdots+a_n}{n}$，证明 $\displaystyle\lim_{n\to\infty}\frac{a_n}{n}=0$。
  \item 证明：$\displaystyle\lim_{n\to\infty}(n!)^{1/n^2}=1$。
  \item 设对每个 $n$ 有 $x_n<1$ 和 $(1-x_n)x_{n+1}\ge\dfrac14$，证明 $\{x_n\}$ 收敛，并求其极限。
  \item 设 $a_1=b$，$a_2=c$，在 $n\ge3$ 时，$a_n=\dfrac{a_{n-1}+a_{n-2}}2$，证明 $\{a_n\}$ 收敛，并求其极限。
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $a_n=\sqrt{1+\sqrt{2+\cdots+\sqrt n}}$，$n\in\NN_+$，证明：$\{a_n\}$ 收敛。
  \item 证明：对每个正整数 $n$，成立不等式
  \[
    \left(1+\frac1n\right)^n>\sum_{k=0}^n\frac1{k!}-\frac\ee{2n}.
  \]
  \item 求极限 $\displaystyle\lim_{n\to\infty}n\sin(2\pi n!\ee)$。
  \item 记 $S_n=1+\dfrac12+\cdots+\dfrac1n$，$n\in\NN_+$。用 $K_n$ 表示使 $S_k\ge n$ 的最小下标，求极限 $\displaystyle\lim_{n\to\infty}\frac{K_{n+1}}{K_n}$。
  \item 设 $x_n=\dfrac1{n^2}\displaystyle\sum_{k=0}^n\ln\binom nk$，$n\in\NN_+$，求 $\displaystyle\lim_{n\to\infty}x_n$。
  \item 将二项式系数 $\binom n0,\binom n1,\cdots,\binom nn$ 的算术平均值和几何平均值分别记为 $A_n$ 和 $G_n$，证明：(1) $\displaystyle\lim_{n\to\infty}\sqrt[n]{A_n}=2$；(2) $\displaystyle\lim_{n\to\infty}\sqrt[n]{G_n}=\sqrt\ee$。
  \item 设 $A_n=\displaystyle\sum_{k=1}^n a_k$，$n\in\NN_+$，数列 $\{A_n\}$ 收敛。又有一个单调增加的正数数列 $\{p_n\}$，且为正无穷大量。证明：
  \[
    \lim_{n\to\infty}\frac{p_1a_1+p_2a_2+\cdots+p_na_n}{p_n}=0.
  \]
  \item 设 $\{a_n\}$ 满足 $\displaystyle\lim_{n\to\infty}\left(a_n\sum_{i=1}^n a_i^2\right)=1$，证明：$\displaystyle\lim_{n\to\infty}\sqrt[3]{3n}\,a_n=1$。
  \item 设数列 $\{u_n\}_{n\ge0}$ 对每个非负整数 $n$ 满足条件
  \[
    u_n=\lim_{m\to\infty}(u_{n+1}^2+u_{n+2}^2+\cdots+u_{n+m}^2),
  \]
  证明：若存在有限极限 $\displaystyle\lim_{n\to\infty}(u_1+u_2+\cdots+u_n)$，则只能是每个 $u_n=0$。
  \item （Toeplitz 定理）设对 $n,k\in\NN_+$，有 $t_{nk}\ge0$。又有 $\displaystyle\sum_{k=1}^n t_{nk}=1$，$\displaystyle\lim_{n\to\infty}t_{nk}=0$。若已知 $\displaystyle\lim_{n\to\infty}a_n=a$，定义 $x_n=\displaystyle\sum_{k=1}^n t_{nk}a_k$，$n\in\NN_+$。证明：$\displaystyle\lim_{n\to\infty}x_n=a$。
  ...
  \item 用 Toeplitz 定理导出 Stolz 定理。
  \item 设 $0<\lambda<1$，$\{a_n\}$ 收敛于 $a$。证明：
  \[
    \lim_{n\to\infty}(a_n+\lambda a_{n-1}+\lambda^2a_{n-2}+\cdots+\lambda^na_0)
    =\frac a{1-\lambda}.
  \]
  \item 设 $\displaystyle\lim_{n\to\infty}x_n=0$，并且存在常数 $K$，使得 $\abs{y_1}+\abs{y_2}+\cdots+\abs{y_n}\le K$ 对每个 $n$ 成立。令 $z_n=x_1y_n+x_2y_{n-1}+\cdots+x_ny_1$，$n\in\NN_+$，证明：$\displaystyle\lim_{n\to\infty}z_n=0$。
  ...
  \item 设 $y_n=x_n+2x_{n+1}$，$n\in\NN_+$。证明：若 $\{y_n\}$ 收敛，则 $\{x_n\}$ 也收敛。
  \item 由初始值 $a_0$ 和 $a_n=2^{n-1}-3a_{n-1}$，$n\in\NN_+$，确定数列 $\{a_n\}$。求 $a_0$ 的所有可能值，使得数列 $\{a_n\}$ 是严格单调增加的。
  \item 证明数列
  \[
  ...
  \item 设参数 $b>0$，$x_1=\dfrac12$，$x_{n+1}=bx_n(1-x_n)$，$n\in\NN_+$。证明以下结论（对于情况 $b>4$ 的讨论即是 2.6.3 小节中的题 3）：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 给定 $x_1,\cdots,x_n$，令 $x_i^{(1)}=\dfrac{x_i+x_{i+1}}2$，$i=1,\cdots,n$，其中 $x_{n+1}=x_1$。归纳地定义
  \[
...
\begin{enumerate}[label=\arabic*.]
  \item 用 $\varepsilon$--$N$ 语言证明 $\displaystyle\lim_{n\to\infty}\frac{a^n}{n!}=0$（注意分情况 $a\ne0,a=0$）。
  \item 用 $\varepsilon$--$N$ 语言证明 $\displaystyle\lim_{n\to\infty}\frac{n^c}{a^n}=0$，其中 $c\ge0,a>1$。
  \item 设 $\displaystyle\lim_{n\to\infty}x_n=a$，又已知用 $\varepsilon$--$N$ 语言描述这个极限时，可取 $N$ 与 $\varepsilon$ 无关，问这样的数列 $\{x_n\}$ 是否一定是常值数列？如果不是，又具有怎样的特性？
  \item
  \begin{enumerate}[label=(\arabic*)]
    \item 正面叙述 $\{x_n\}$ 不是无穷小量（用对偶法则）。
    \item 正面叙述“$\forall\varepsilon>0,\exists N,\forall n,m\ge N$ 有 $\abs{x_n-x_m}<\varepsilon$”的否命题。
  \end{enumerate}
  \item 证明：给定实数 $a$ 的任意邻域 $O_\delta(a)$ 中必定同时存在有理数与无理数。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  \item 求 $\displaystyle\lim_{n\to\infty}(\lambda+2\lambda^2+\cdots+n\lambda^n)$，其中 $\abs\lambda<1$。
  \item 求 $\displaystyle\lim_{n\to\infty}\left(\frac1{\sqrt{n^2+1}}+\cdots+\frac1{\sqrt{n^2+n}}\right)$。
  \item 设 $b_n>0$，$\displaystyle\lim_{n\to\infty}\frac{b_{n+1}}{b_n}=b$。证明：$\displaystyle\lim_{n\to\infty}b_n^{1/n}=b$。

  （$b=0$ 时，用夹逼定理；$b>0$ 时，取对数。）
  \item 证明对应于 $\displaystyle\lim_{n\to\infty}x_n=+\infty$ 的 Cauchy 命题。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \begin{align*}
      ...
      &=\lim_{n\to\infty}\frac1{(n+1)^2}+\lim_{n\to\infty}\frac1{(n+2)^2}+\cdots+\lim_{n\to\infty}\frac1{(2n)^2}\\
      &=0+0+\cdots+0=0
    \end{align*}
    （错用收敛数列的四则运算法则）；
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $0<x_0<1$，$x_{n+1}=1-\sqrt{1-x_n}$，$n\in\NN_+$，求 $\displaystyle\lim_{n\to\infty}x_n$。
  \item 求下列极限：
  \begin{enumerate}[label=(\arabic*)]
    \item $\displaystyle\lim_{n\to\infty}\left(1+\frac1{2n}\right)^n$；
    \item $\displaystyle\lim_{n\to\infty}\left(1+\frac1{n^2}\right)^n$。
  \end{enumerate}
  \item 证明：$\{a_n\}$ 收敛 $\Longleftrightarrow$ $\{a_{2n}\}$ 和 $\{a_{2n+1}\}$ 收敛到同一极限。
  \item 求极限
  \[
...
\begin{enumerate}[label=\arabic*.]
  \item 设 $A$ 和 $B$ 是上有界集，定义 $A+B=\{x+y\mid x\in A,y\in B\}$。证明 $\sup(A+B)=\sup A+\sup B$。
  \item 设 $\displaystyle\lim_{n\to\infty}x_n=a>0$，证明：存在 $N$，使当 $n>N$ 时，$x_n>0$。并举例说明逆命题不成立。
  \item 求 $\displaystyle\lim_{n\to\infty}\left(\frac1{n+\sqrt1}+\frac1{n+\sqrt2}+\cdots+\frac1{n+\sqrt n}\right)$。
  \item 叙述并证明 $\dfrac00$ 型不定式的 Stolz 定理。
  \item 证明极限 $\displaystyle\lim_{n\to\infty}\sqrt{1+\sqrt{2+\cdots+\sqrt n}}$（$n$ 重根号）的存在性。
\end{enumerate}
