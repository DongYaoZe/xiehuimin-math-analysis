...
\begin{xthought}
\begin{enumerate}
  \item 下列几种叙述能否作为函数极限 $\lim_{x\to a}f(x)=A$ 的等价定义？
  ...
  \end{enumerate}
  \item 用对偶法则给出：(1)“$f(x)$ 在点 $a$ 不收敛于 $A$”的正面叙述；(2)“$f(x)$ 在点 $a$ 处没有极限”的正面叙述。
  \item 怎样用正面方式叙述下列否定性概念：
  \begin{enumerate}[label=(\arabic*)]
...
\begin{xexercise}
\begin{enumerate}
  \item 证明：$\displaystyle\lim_{x\to0}\frac{\sqrt{1+x}-\sqrt{1-x}}{x}=1$。
  \item 证明：$\displaystyle\lim_{x\to1}\frac{x^2+x-2}{x(x^2-3x+2)}=-3$。
  \item 证明：$\displaystyle\lim_{x\to+\infty}\frac{x+1}{x^2-x}=0$。
  \item 当 $a$ 取什么数值时，$\displaystyle\lim_{x\to-1}\frac{x^3-ax^2-x+4}{x+1}$ 存在？此时极限为何？
  \item 求 $a,b$，使 $\displaystyle\lim_{x\to2}\frac{x^2+ax+b}{x^2-x-2}=2$。
  \item 问：使得 $\displaystyle\lim_{x\to0^+}\frac{a+\sin\frac1x}{x}=\pm\infty$ 的参数 $a$ 是什么？
  \item 证明：$\displaystyle\lim_{x\to a}\ln x=\ln a$，其中 $a>0$。
  \item 证明：$\displaystyle\lim_{x\to a}\ee^x=\ee^a$。
  \item 证明 $\displaystyle\lim_{x\to0}f(x)$ 与 $\displaystyle\lim_{x\to0}f(x^3)$ 同时存在或不存在，而当它们存在时必相等。
  \item 问 $\displaystyle\lim_{x\to0}f(x)$ 与 $\displaystyle\lim_{x\to0}f(x^2)$ 是否一定同时存在或不存在？
  \item 证明：Dirichlet（狄利克雷）函数
  \[
    D(x)=\begin{cases}
      1,&x\text{ 是有理数},\\
      0,&x\text{ 是无理数}
  ...
  \item 试举出一个在区间 $(-\infty,+\infty)$ 上定义的函数，使得它在点 $x=1$ 处有极限，但在区间的其他点都没有极限。
  \item 证明：若 $f$ 为周期函数，且 $\lim_{x\to+\infty}f(x)=0$，则 $f(x)\equiv0$。
...
\begin{xthought}
\begin{enumerate}
  \item 试就 $\lim_{x\to+\infty}f(x)=A$ 和 $\lim_{x\to a^+}f(x)=A$ 两类极限叙述极限的唯一性定理、局部有界性定理、局部保号性定理、比较定理、夹逼定理、Heine 归结原理和 Cauchy 收敛准则。
  \item 回答下列有关极限的四则运算法则方面的问题：
  \begin{enumerate}
  ...
  \end{enumerate}
  \item 对于极限的加法运算法则作出两个证明：(1) 用函数极限定义；(2) 用 Heine 归结原理。
  \item 对于各种类型的函数极限中 $A=\infty$ 但不是有确定符号的无穷大量的情况，夹逼定理不成立。为什么？举出反例。
\end{enumerate}
\end{xthought}
...
\begin{enumerate}
  ...
  \end{enumerate}
  \item 求 $\displaystyle\lim_{y\to+\infty}\frac{\sqrt{1+y^3}}{\sqrt{y^2+y^3}+y}$。
  \item 求 $\displaystyle\lim_{x\to+\infty}\left(\frac{x^2-1}{x^2+1}\right)^{\frac{x-1}{x+2}}$。
  \item 求 $\displaystyle\lim_{x\to0}\frac{\sqrt[n]{1+x}-1}{x}$，其中 $n$ 为正整数。
  \item 设已知 $\displaystyle\lim_{x\to0}\frac{f(x)}x=l,b\ne0$，求 $\displaystyle\lim_{x\to0}\frac{f(bx)}x$。
  \item 证明：$\displaystyle\lim_{x\to0}\frac{\sqrt{1+\sin x}-\sqrt{1-\sin x}}{\sin x}=1$。
  \item 证明：在区间 $(a,+\infty)$ 上单调有界函数 $f$ 一定存在极限 $\lim_{x\to+\infty}f(x)$。
  \item 设 $f(x)$ 在区间 $(a,b)$ 上为单调增加函数，且存在一个数列 $\{x_n\}\subset(a,b)$，使得 $\lim_{n\to\infty}x_n=b,\lim_{n\to\infty}f(x_n)=A$。证明：(1) $f$ 在区间 $(a,b)$ 上以 $A$ 为上界；(2) $\lim_{x\to b^-}f(x)=A$。
  \item 设 $\lim_{x\to+\infty}f(x)=A>0$。证明：对每个 $c\in(0,A)$，存在 $M>0$，当 $x>M$ 时，成立 $f(x)>c$。
...
\subsection{$\displaystyle\lim_{x\to0}\frac{\sin x}{x}=1$}
...
\subsection{$\displaystyle\lim_{x\to0}(1+x)^{1/x}=\ee$}
...
\begin{align*}
  ...
  =\lim_{x\to\infty}\left[1+\left(\sin\frac1x+\cos\frac1x-1\right)\right]^{\frac{x}{\sin(1/x)+\cos(1/x)-1}(\sin(1/x)+\cos(1/x)-1)}.
\end{align*}
可见只要计算出
\[
...
\begin{enumerate}
  ...
  \item 设 $a_1,\ldots,a_n$ 为正数，$n\geq2$，
  \[
    f(x)=\left(\frac{a_1^x+a_2^x+\cdots+a_n^x}{n}\right)^{1/x},
  \]
  求 $\displaystyle\lim_{x\to0}f(x)$。
  \item 计算极限 $\displaystyle\lim_{n\to\infty}\prod_{k=1}^n\cos\frac{x}{2^k}$，并证明 Viète（韦达）公式
  \[
    \frac\pi2=
    \frac{1}{
      \sqrt{\frac12}\cdot
...
\subsection{记号 $\littleo,\bigO$ 与 $\sim$}
...
\begin{xthought}
\begin{enumerate}
  \item $10^{-10,000},\ee^{-10^{10}},x,\sin x$ 是否是无穷小量？$10^{10,000},\ee^{10^{10}},x^n,a^x\ (a>1)$ 是否是无穷大量？
  \item 确定下列极限是否存在，若存在，等于什么？（观察图 4.2 中的图像。）
  \begin{enumerate}
  ...
  \end{enumerate}
  \item 作出 $y=\ee^{1/x}$ 的图形，观察：$\ee^{1/x}\to+\infty\ (x\to0^+)$ 和 $\ee^{1/x}=\littleo(1)\ (x\to0^-)$。
\end{enumerate}
\end{xthought}
...
\begin{enumerate}
  ...
  \end{enumerate}
  \item 设存在极限 $\displaystyle\lim_{x\to0}\frac{f(x)}x$，又有 $f(x)-f\left(\frac x2\right)=\littleo(x)\ (x\to0)$，证明：$f(x)=\littleo(x)\ (x\to0)$。
  \item 与数列中的几个常见的无穷大量之间的关系 $\ln n\ll n^\varepsilon\ll a^n\ll n!\ll n^n\ (a>1,\varepsilon>0)$ 相类似，证明当 $x\to+\infty$ 时，有
  \[
...
\begin{enumerate}
  \item 若函数 $f$ 在区间 $(a,b)$ 上单调，且有一个数列 $\{x_n\}$ 使得 $x_n\to a^+$ 和 $\lim_{n\to\infty}f(x_n)=A$。请按照单侧极限的定义直接证明：$\lim_{x\to a^+}f(x)=A$。
  \item 设函数 $f$ 在区间 $[a,b]$ 上严格单调增加，且有一个在区间 $[a,b]$ 内的数列 $\{x_n\}$ 使得 $f(x_n)\to f(a)$。证明：$\lim_{n\to\infty}x_n=a$。
  \item 在 Heine 归结原理（命题 4.2.3）的条件中，
  \begin{enumerate}
  ...
  \item 证明 $\sin\sqrt{x+1}-\sin\sqrt{x}$ 当 $x\to+\infty$ 时极限为 0，并分析其阶数。
  \item 求 $\displaystyle\lim_{x\to+\infty}\left(\frac1x\cdot\frac{a^x-1}{a-1}\right)^{1/x}$，其中 $a>0,a\ne1$。
  \item
  \begin{enumerate}
  ...
  \end{enumerate}
  \item 对一般的正整数 $n$ 计算极限 $\displaystyle\lim_{x\to0}\frac{\sin nx-n\sin x}{x^3}$。
  \item 证明 Dirichlet 函数（4.1.5 小节的题 11）有以下解析表达式：
  \[
  ...
  \end{enumerate}
  \item 设函数 $f$ 在 $\RR$ 上定义，在 $x=0$ 邻近有界，又有 $a>1,b>1$，使得对每个 $x\in\RR$ 成立 $f(ax)=bf(x)$。证明：$\lim_{x\to0}f(x)=0$。
  \item 设函数 $f$ 在 $(0,+\infty)$ 上单调增加，且有 $\displaystyle\lim_{x\to+\infty}\frac{f(2x)}{f(x)}=1$。证明：对每个 $a>0$，成立 $\displaystyle\lim_{x\to+\infty}\frac{f(ax)}{f(x)}=1$。
  \item 设 $f$ 在 $(0,+\infty)$ 上定义，且在其中的每个有界子区间上有界。证明：等式
  \[
  ...
  \]
  在右边为有限极限或 $\pm\infty$ 时成立。
  \item 设 $T$ 为正整数，若函数 $f,g$ 在 $[a,+\infty)$ 上满足条件：
  \begin{enumerate}
  ...
  \item 设成立 $\displaystyle\lim_{x\to0}f(x)=0$，$f(x)-f\left(\frac x2\right)=\littleo(x)\ (x\to0)$。证明：$f(x)=\littleo(x)\ (x\to0)$。（本题比 4.4.4 小节的题 2 要难一点。）
\end{enumerate}
