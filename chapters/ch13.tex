...
\item （例题 13.1.2 之续）设比赛开始时，乌龟在 Achilles 之前 $1000\,\mathrm m$，Achilles 的速度为 $10\,\mathrm{m/s}$，乌龟的爬行速度为 $0.1\,\mathrm{m/s}$。试将 Zeno 悖论中的论点转化为无穷级数求和问题，并计算出 Achilles 赶上乌龟所需要的时间。（本题表明：有了无穷级数的工具之后，Zeno 的这个悖论就不再是悖论了。）
\item 设级数 $\sum a_n$ 与 $\sum b_n$ 均收敛（或发散），讨论下列级数的敛散性：
\[
\sum(a_n\pm b_n),\qquad \sum a_nb_n,\qquad \sum\frac{a_n}{b_n}\quad(b_n\neq0),
\]
又对于 $\sum a_n$ 和 $\sum b_n$ 均为正项级数情况讨论同样的问题。
\item 对于级数 $\sum\max\{a_n,b_n\}$，$\sum\min\{a_n,b_n\}$ 继续上题的讨论。
\item 如果级数 $\sum(a_n+a_{n+1})$ 收敛，是否必可推出 $\sum a_n$ 收敛？在 $\sum a_n$ 是正项级数时，答案又是什么？
\item 记 $S=1-1+1-1+\cdots$，并作以下计算：
\[
S=1-(1-1+1-1+\cdots)=1-S\Rightarrow S=\frac12,
\]
问：其中有何错误？又可在阅读第二组参考题 18 后再做此题。
\item 讨论级数 $a_1-a_1+a_2-a_2+\cdots+a_n-a_n+\cdots$ 的敛散性。
\item 如果对每个正整数 $p$ 均成立
\[
\lim_{n\to\infty}(a_{n+1}+a_{n+2}+\cdots+a_{n+p})=0,
\]
问：是否能由此推出级数 $\sum a_n$ 一定收敛？
\item 设 $a_n<c_n<b_n$，$n=1,2,\ldots$，若已知级数 $\sum a_n$ 和 $\sum b_n$ 同时收敛（或发散），问：能否推出级数 $\sum c_n$ 收敛（或发散）？并请将你的结论与数列的夹逼定理（见上册 20 页）作比较。
\item 设 $\sum a_n$ 收敛于和 $S$，若对于每个 $n$ 将 $a_{2n-1}$ 和 $a_{2n}$ 两项作交换，证明所得的新级数收敛，并求其和。
\item 设有收敛正项级数 $\sum a_n$，且对于每个正整数 $n$ 成立 $a_n\leq a_nR_n$，其中 $R_n$ 为第 $n$ 个余项，证明：$\sum a_n$ 实际上是有限和。
\end{enumerate}
\end{xthought}
...
\[
\begin{array}{lll}
\text{Cauchy 根值判别法：}&\displaystyle \lim_{n\to\infty}\sqrt[n]{a_n}=c,&c<1\text{ 时级数收敛，}\ c>1\text{ 时级数发散};\\[6pt]
\text{d'Alembert 判别法：}&\displaystyle \lim_{n\to\infty}\frac{a_{n+1}}{a_n}=d,&d<1\text{ 时级数收敛，}\ d>1\text{ 时级数发散};\\[6pt]
\text{Raabe 判别法：}&\displaystyle \lim_{n\to\infty}n\left(\frac{a_n}{a_{n+1}}-1\right)=r,&r>1\text{ 时级数收敛，}\ r<1\text{ 时级数发散};\\[6pt]
\text{Bertrand 判别法：}&\displaystyle \lim_{n\to\infty}\ln n\left[n\left(\frac{a_n}{a_{n+1}}-1\right)-1\right]=b,&b>1\text{ 时级数收敛，}\ b<1\text{ 时级数发散};\\[6pt]
\text{Gauss 判别法：}&\displaystyle \frac{a_n}{a_{n+1}}=1+\frac{\mu}{n}+\bigO\left(\frac1{n^{1+\varepsilon}}\right),\ \varepsilon>0,&\mu>1\text{ 时级数收敛，}\ \mu\leq1\text{ 时级数发散}.
\end{array}
\]
...
\begin{xexercise}
\begin{enumerate}[label=\arabic*.]
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 和 $\sum_{n=1}^{\infty}b_n$ 的通项为等价量：$a_n\sim b_n$，又记两个级数的部分和为 $S_n$ 和 $S'_n$，余项为 $R_n$ 和 $R'_n$，$n=1,2,\ldots$，证明：
\begin{enumerate}[label=(\arabic*)]
\item 若两个级数均收敛，则 $R_n\sim R'_n$；
\item 若两个级数均发散，则 $S_n\sim S'_n$。
\end{enumerate}
\item 设 $\sum a_n$ 为正项级数，证明：任意改变其中各项的顺序和加括号后得到的级数与原来的级数具有相同的敛散性，且在收敛时级数的和不变。（这表明，如果无限项求和时每项为非负数，或至多只有有限项为负数，则有限项求和时的结合律和交换律对于无限项求和都依然成立。）
\item 讨论下列各级数的敛散性：
\[
\begin{array}{ll}
...
\end{array}
\]
\item 从例题 13.2.6 和 13.2.7 知道，若从收敛正项级数 $\sum_{n=1}^{\infty}a_n$ 作出 $\{u_n\}$，其中 $u_n$ 为 $a_1,a_2,\ldots,a_n$ 的调和平均值或几何平均值，则 $\sum_{n=1}^{\infty}u_n$ 收敛。但是若 $u_n$ 为算术平均值，则情况大不一样。证明：若正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛，则除了一种特殊情况之外，级数 $\sum_{n=1}^{\infty}\frac{a_1+a_2+\cdots+a_n}{n}$ 必定发散。
\item 试用 Sapagof 判别法（即命题 13.2.3）或其他方法证明下列数列收敛于 0：
\[
(1)\ \left\{\frac{(2n)!}{4^n(n!)^2}\right\};\qquad
(2)\ \left\{\frac{n^n}{\ee^n n!}\right\};\qquad
(3)\ \left\{\frac{n!}{(x+1)(x+2)\cdots(x+n)}\right\}\ (x>0).
\]
\item 举出一个收敛的正项级数 $\sum a_n$ 的例子，使它满足条件 $\limsup_{n\to\infty}na_n=1$。
\item 已知正项级数 $\sum a_n$ 收敛，$p>1$，证明：级数 $\sum a_n^p$ 一定收敛。又若去掉 $\sum a_n$ 为正项级数的条件，结论是否还成立？
\item 设数列 $\{a_n\}$ 单调减少收敛于 0，且 $b_n=a_n-2a_{n+1}+a_{n+2}>0$，$n=1,2,\ldots$；证明：级数 $\sum_{n=1}^{\infty}nb_n$ 收敛，且其和为 $a_1$。
\item 设对于每个正整数 $n$ 都有 $0<a_n<a_{2n}+a_{2n+1}$，证明：级数 $\sum_{n=1}^{\infty}a_n$ 发散。
\item 设 $x>0$，证明：级数
\[
...
\]
的敛散性。
\item 设 $\sum_{n=1}^{\infty}a_n$ 为正项级数，证明：
\begin{enumerate}[label=(\arabic*)]
...
\end{enumerate}
\item 设 $\sum_{n=1}^{\infty}a_n$ 为正项级数，$k$ 为一正整数，如果 $\lim_{n\to\infty}\frac{a_{n+k}}{a_n}=l$，证明：$l<1$ 时，$\sum a_n$ 收敛；$l>1$ 时，$\sum a_n$ 发散。（本题如取 $k=1$，就是 d'Alembert 判别法。因此本题可看作为 d'Alembert 判别法的一个推广。）
\item （Dini（迪尼））设正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛，其余项为 $R_n=\sum_{k=n+1}^{\infty}a_k$，$n=0,1,\ldots$。证明：$\sum_{n=1}^{\infty}\frac{a_n}{R_{n-1}}$ 发散，但对任意 $p>0$，$\sum_{n=1}^{\infty}\frac{a_n}{R_{n-1}^{1-p}}$ 收敛。
\item 设 $0<a_1<\frac\pi2$，然后用迭代公式 $a_n=\sin a_{n-1}$ 生成数列 $\{a_n\}$，讨论级数 $\sum_{n=1}^{\infty}a_n^p$ 的敛散性（参考上册 233 页的例题 8.1.10）。
\end{enumerate}
\end{xexercise}
...
\end{array}
\]
\item 将调和级数 $\sum_{n=1}^{\infty}1/n$ 的各项改变符号，其规则是每 $p$ 个正项后为 $q$ 个负项，然后再如此重复，但不改变各项原有的顺序。证明：所得级数当且仅当 $p=q$ 时收敛。
\item 设 $0<a<1$，将级数 $\sum_{n=1}^{\infty}\frac{(-1)^{n-1}}{n^a}$ 按照每 $p$ 个正项后为 $q$ 个负项的规则进行重排，但保持正项之间和负项之间的原有顺序，证明：重排后的级数当且仅当 $p=q$ 时收敛。
\item 设 $p,q>0$，讨论下列级数的绝对收敛性与条件收敛性：
\[
...
\]
\item 研究级数 $\sum_{n=1}^{\infty}\frac1{1+nx^n}$（若分母为零则去掉该项）的条件收敛性和绝对收敛性。
\item 设数列 $\{a_n\}$ 严格单调减少且趋于零，证明下列级数收敛：
\[
...
\]
\item （Du Bois-Reymond 判别法）设级数 $\sum_{n=2}^{\infty}(a_n-a_{n-1})$ 绝对收敛，级数 $\sum_{n=1}^{\infty}b_n$ 收敛，证明：$\sum_{n=1}^{\infty}a_nb_n$ 收敛。
\item （Dedekind 判别法）设级数 $\sum_{n=2}^{\infty}(a_n-a_{n-1})$ 绝对收敛，$\lim_{n\to\infty}a_n=0$，级数 $\sum_{n=1}^{\infty}b_n$ 的部分和数列有界，证明：$\sum_{n=1}^{\infty}a_nb_n$ 收敛。
\item 已知级数 $\sum_{n=1}^{\infty}a_n$ 发散，证明：级数 $\sum_{n=1}^{\infty}(1+1/n)a_n$ 也发散。
\item 证明：当 $|q|<1$ 时，成立
\[
\left(\sum_{n=0}^{\infty}q^n\right)\left(\sum_{n=0}^{\infty}q^n\right)=\sum_{n=0}^{\infty}(n+1)q^n.
\]
\item 证明：级数 $\sum_{n=0}^{\infty}\frac{(-1)^n}{n+1}$ 与自身的 Cauchy 乘积是收敛级数。
\item 设
\[
S(x)=\sum_{n=0}^{\infty}\frac{(-1)^nx^{2n+1}}{(2n+1)!},\qquad C(x)=\sum_{n=0}^{\infty}\frac{(-1)^nx^{2n}}{(2n)!},
\]
证明：$2S(x)C(x)=S(2x)$。
\item 设级数 $\sum a_n/b_n$、$\sum(a_n/b_n)^2$ 均收敛，又对每个 $n$ 有 $b_n(a_n+b_n)\neq0$，证明：级数 $\sum\frac{a_n}{a_n+b_n}$ 收敛。
\item 两个发散级数的 Cauchy 乘积可以是收敛的，试验证下列两个发散级数
\[
...
\end{array}
\]
\item 设 $a,b>0$，求 $\lim_{n\to\infty}\frac{(a+1)(a+2)\cdots(a+n)}{(b+1)(b+2)\cdots(b+n)}$。
\item 设数列 $\{a_n\}$ 的每一项满足条件 $0<a_n<\pi/2$，证明：级数 $\sum_{n=1}^{\infty}a_n^2$ 收敛的充分必要条件是无穷乘积 $\prod_{n=1}^{\infty}\cos a_n$ 收敛。
\item 如果对正数数列 $\{a_n\}$ 存在极限
\[
...
\item （Stirling 公式的又一证明）用无穷乘积方法证明：数列
\[
...
\begin{enumerate}[label=\arabic*.]
\item 试用两种方法证明：对于正数数列 $\{a_n\}$，必有
\[
\lim_{n\to\infty}\frac{a_n}{(1+a_1)(1+a_2)\cdots(1+a_n)}=0.
\]
\item （Abel-Pringsheim（普林斯海姆）定理）设正项级数 $\sum a_n$ 的通项单调减少，且已知级数收敛，证明：$\lim_{n\to\infty}na_n=0$。（举例说明，若通项非单调，则结论不成立。）
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 的通项单调减少，又已知级数 $\sum_{n=1}^{\infty}\frac{a_n}{\sqrt n}$ 收敛，证明：级数 $\sum_{n=1}^{\infty}a_n^2$ 收敛。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 的通项单调减少收敛于 0。(1) 证明：级数 $\sum_{n=1}^{\infty}a_n$ 与级数 $\sum_{n=1}^{\infty}n(a_n-a_{n+1})$ 同敛散，且在收敛时具有相同的和。(2) 对于通项不是单调减少的情况，试举例说明上述“同敛散”的结论不再成立。
\item 由于数列 $\{a_n\}$ 与无穷级数 $\sum_{n=1}^{\infty}(a_{n+1}-a_n)$ 具有相同的敛散性，试用级数理论讨论下列数列的敛散性：
\begin{enumerate}[label=(\arabic*)]
...
\item 证明：(1) 若 $f$ 是 $[1,+\infty)$ 上的非负单调减少函数，则存在极限
\[
...
\]
\item 若函数 $f$ 在 $[-1,1]$ 上定义，且存在 $f''(0)$，证明：级数 $\sum_{n=1}^{\infty}f(1/n)$ 绝对收敛的充分必要条件是 $f(0)=f'(0)=0$。
\item 设函数 $f$ 在 $[1,+\infty)$ 上单调增加且有极限 $f(+\infty)=A$。证明：(1) $\sum_{n=1}^{\infty}[f(n+1)-f(n)]$ 收敛，并求其和；(2) 又若 $f$ 在 $[1,+\infty)$ 上二次可微，且 $f''(x)<0$，则 $\sum_{n=1}^{\infty}f'(n)$ 也收敛。
\item （二重正项级数的求和顺序交换定理）设对每个正整数 $n$，正项级数 $\sum_{k=1}^{\infty}a_{n,k}$ 收敛，且其和为 $a_n$；若级数 $\sum_{n=1}^{\infty}a_n$ 收敛，证明：
\[
\sum_{n=1}^{\infty}a_n=\sum_{n=1}^{\infty}\sum_{k=1}^{\infty}a_{n,k}=\sum_{k=1}^{\infty}\sum_{n=1}^{\infty}a_{n,k}.
\]
\item 对正整数 $m\geq2$，用 $s_m$ 表示级数 $\sum_{n=2}^{\infty}\frac1{n^m}$ 的和，证明：$\sum_{m=2}^{\infty}s_m=1$。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 的通项单调减少趋于 0，且已知数列
\[
\left\{\sum_{k=1}^{n}(a_k-a_n)\right\}
\]
有界，证明：$\sum_{n=1}^{\infty}a_n$ 收敛。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛，且数列 $\{a_n-a_{n+1}\}$ 严格单调减少，证明：
\[
\lim_{n\to\infty}\left(\frac1{a_{n+1}}-\frac1{a_n}\right)=+\infty.
\]
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛，数列 $\{na_n\}$ 单调，证明：$\lim_{n\to\infty}na_n\ln n=0$。
\item 设 $a_1=1$，$a_2=2$，$a_n=a_{n-1}+a_{n-2}$，$n=3,4,\ldots$，证明：$\sum_{n=1}^{\infty}1/a_n$ 收敛。（Fibonacci 数列的倒数所成级数收敛。）
\item 设 $0<p<1$，$a_1>0$，$a_{n+1}=\frac{a_n}{1+a_n^p}$，$n=1,2,\ldots$，证明：$\sum_{n=1}^{\infty}a_n$ 收敛。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛，则对于 $p>\frac12$，证明：级数 $\sum_{n=1}^{\infty}\frac{\sqrt{a_n}}{n^p}$ 收敛。举例说明 $p=\frac12$ 时级数 $\sum_{n=1}^{\infty}\frac{\sqrt{a_n}}{n^{1/2}}$ 可能发散。
\item 证明下列两个级数发散：(1) $\sum_{n=1}^{\infty}\sin n$；(2) $\sum_{n=1}^{\infty}\sin n^2$。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 发散，讨论下列级数的敛散性：
\[
...
\]
\item 设有收敛级数 $\sum_{n=1}^{\infty}a_n=S$，证明：(1)
\[
...
\]
收敛，且其和也是 $S$。
\item 设级数 $\sum_{n=1}^{\infty}a_n=s$ 为条件收敛，$\sum_{n=1}^{\infty}a_{f(n)}=t$ 是 $\sum_{n=1}^{\infty}a_n$ 的一个重排。若 $t\neq s$，证明：对于任意 $N$，存在 $n$，使 $|n-f(n)|>N$。（这表明，对于条件收敛级数作重排时，如果每一项的新位置与原有位置之差不超过某个界限的话，则级数的和不会改变。）
\end{enumerate}
...
\begin{xreferenceproblem}
\begin{enumerate}[label=\arabic*.]
\item 设正项级数 $\sum_{n=1}^{\infty}\frac1{a_n}$ 收敛，证明：级数
\[
\sum_{n=1}^{\infty}\frac{n^2a_n}{(a_1+a_2+\cdots+a_n)^2}
\]
收敛。
\item 设 $a>0$，$a_n>0$，$n=1,2,\ldots$，证明：级数
\[
\sum_{n=1}^{\infty}\frac{a_n}{(a+a_1+\cdots+a_n)^{3/2}}
\]
收敛，且其和 $S\leq2/\sqrt a$。
\item 设正项级数 $\sum_{n=1}^{\infty}a_n$ 收敛且存在 $M>0$，使对每个正整数 $n$ 成立
\[
a_k\leq Ma_n,\qquad n\leq k\leq2n,
\]
证明：$\lim_{n\to\infty}na_n=0$。
\item 试构造两个发散的正项级数 $\sum a_n$ 与 $\sum b_n$，它们的通项都单调减少，但级数 $\sum_{n=1}^{\infty}\min\{a_n,b_n\}$ 收敛。
\item （Frink（弗林克）判别法）设对正项级数 $\sum_{n=1}^{\infty}a_n$ 存在极限
\[
\lim_{n\to\infty}\left(\frac{a_{n+1}}{a_n}\right)^n=\lambda,
\]
则当 $\lambda<\ee^{-1}$ 时级数收敛，而当 $\lambda>\ee^{-1}$ 时级数发散。
\item （Ermakov（叶尔马科夫）判别法）设 $f$ 在 $[a,+\infty)$ 上单调减少且大于 0，又知存在极限
\[
\lim_{x\to+\infty}\frac{\ee^xf(\ee^x)}{f(x)}=p,
\]
则正项级数 $\sum_{n=1}^{\infty}f(n)$ 当 $p<1$ 时收敛，而当 $p>1$ 时发散。
\item （Lobachevski（罗巴切夫斯基）判别法）通项单调趋于 0 的正项级数 $\sum_{n=1}^{\infty}a_n$ 与级数 $\sum_{n=1}^{\infty}b_n2^{-n}$ 同敛散，其中 $b_n=\max\{k\mid a_k\geq2^{-n}\}$。
\item 证明：下列两个无穷乘积收敛于自然对数的底 $\ee$：
\begin{enumerate}[label=(\arabic*)]
...
\item 证明：若 $p\leq1$，则级数 $\sum_{n=1}^{\infty}\frac{\cos(\ln n)}{n^p}$ 发散。
\item 设级数 $\sum_{n=1}^{\infty}na_n$ 收敛，证明：(1) 对任一正整数 $k$，级数 $\sum_{n=1}^{\infty}na_{n+k}$ 收敛；(2)
\[
\lim_{k\to\infty}\sum_{n=1}^{\infty}na_{n+k}=0.
\]
\item 若对每一个满足 $\lim_{n\to\infty}b_n=0$ 的数列 $\{b_n\}$，级数 $\sum_{n=1}^{\infty}a_nb_n$ 收敛，证明：级数 $\sum_{n=1}^{\infty}a_n$ 绝对收敛。
\item 若对每一个收敛级数 $\sum_{n=1}^{\infty}b_n$，级数 $\sum_{n=1}^{\infty}a_nb_n$ 收敛，证明：级数 $\sum_{n=1}^{\infty}|a_n-a_{n+1}|$ 必定收敛。
\item 举出一个收敛级数 $\sum_{n=1}^{\infty}a_n$，使得级数 $\sum_{n=1}^{\infty}a_n^3$ 发散。
\item 设函数 $f$ 在 $[1,+\infty)$ 上连续可微，且 $\int_1^{+\infty}|f'(x)|\,\dd x$ 收敛，证明：广义积分 $\int_1^{+\infty}f(x)\,\dd x$ 与无穷级数 $\sum_{n=1}^{\infty}f(n)$ 同敛散。
\item 设 $p>0$，证明：交错级数 $\sum_{n=1}^{\infty}\frac{(-1)^{n-1}}{n^p}$ 的和 $S$ 满足估计 $\frac12<S<1$。
\item 设级数 $\sum_{n=1}^{\infty}a_nb_n$ 收敛，又知 $\{b_n\}$ 单调减少收敛于 0，证明：
\[
\lim_{n\to\infty}(a_1+a_2+\cdots+a_n)b_n=0.
\]
（第一组参考题 19 之 (1) 为本题之特例。）
\item 设级数 $\sum_{n=1}^{\infty}\frac{a_n}{n^p}$ 收敛，其中 $p>0$，证明：
\[
\lim_{n\to\infty}\frac{a_1+a_2+\cdots+a_n}{n^p}=0.
\]
\item 设级数 $\sum_{n=1}^{\infty}a_n$ 的部分和数列为 $\{S_n\}$，并令
\[
...
\item 证明：级数 $\sum_{n=1}^{\infty}a_n$ 可以 Cesàro 求和的必要条件是
\[
\lim_{n\to\infty}\frac{a_n}{n}=0.
\]
（这个条件比通常意义下级数 $\sum a_n$ 收敛时通项必须为无穷小量的必要条件弱得多。）
\item 下列级数是否可以 Cesàro 求和？如果可以，试求出它们的 Cesàro 和。
\[
...
\]
\item 设级数 $\sum_{n=1}^{\infty}a_n$ 的通项满足条件 $\lim_{n\to\infty}na_n=0$，证明：该级数在通常意义下收敛的充分必要条件是级数可以 Cesàro 求和。
\item 设级数 $\sum_{n=1}^{\infty}a_n$ 可以 Cesàro 求和，证明：该级数在通常意义下收敛的充分必要条件是
\[
\lim_{n\to\infty}\frac{a_1+2a_2+\cdots+na_n}{n}=0.
\]
\end{enumerate}
