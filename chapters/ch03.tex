...
\[
  a_1\geqslant a_2\geqsl\cdots\geqslant a_n\geqslant\cdots,
\]
...
\begin{enumerate}
  ...
  \item 设数集 $A$ 以 $\beta$ 为上界，又有数列 $\{x_n\}\subset A$ 和 $\lim_{n\to\infty}x_n=\beta$。证明：$\beta=\sup A$。
  \item 求下列数集的上确界和下确界：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \end{enumerate}
  \item 设有两个数集 $A$ 和 $B$，且对数集 $A$ 中的任何一个数 $x$ 和数集 $B$ 中的任何一个数 $y$ 成立不等式 $x<y$。证明：$\sup A\leqslant\inf B$。
  \item 设数集 $A$ 有上界，数集 $B=\{x+c\mid x\in A\}$，其中 $c$ 是一个常数。证明：
  \[
    \sup B=\sup A+c;\qquad \inf B=\inf A+c.
  \]
  \item 设 $A,B$ 是两个有上界的数集，又有数集 $C\subset\{x+y\mid x\in A,y\in B\}$，则 $\sup C\leqslant\sup A+\sup B$。举出成立严格不等号的例子。
  \item 设 $A,B$ 是两个有上界的数集，又有数集 $C\supset\{x+y\mid x\in A,y\in B\}$，则 $\sup C\geqslant\sup A+\sup B$。举出成立严格不等号的例子。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item 如闭区间套定理中的闭区间套改为开区间套 $\{(a_n,b_n)\}$，其他条件不变，则可以举出例子说明结论不成立。
  \item 如 $\{(a_n,b_n)\}$ 为开区间套，数列 $\{a_n\}$ 严格单调增加，数列 $\{b_n\}$ 严格单调减少，又满足条件 $a_n<b_n$，$n\in\NN_+$，证明 $\bigcap_{n=1}^{\infty}(a_n,b_n)\neq\varnothing$。
...
\begin{enumerate}
  ...
  \item 证明：有界数列发散的充分必要条件是存在两个收敛于不同极限值的子列。
...
\begin{xexercise}
\begin{enumerate}
  \item 满足以下条件的数列 $\{x_n\}$ 是否一定是基本数列？若回答“是”，请作出证明；若回答“不一定是”，请举出反例：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \end{enumerate}
  \item 设 $a_n=\sin1+\frac{\sin2}{2!}+\cdots+\frac{\sin n}{n!}$，$n\in\NN_+$，证明：(1) 数列 $\{a_n\}$ 有界，但不单调；(2) $\{a_n\}$ 收敛。
  \item 设从某个数列 $\{a_n\}$ 定义
  \[
  ...
  \item 设 $S_n=1+\frac1{2^p}+\frac1{3^p}+\cdots+\frac1{n^p}$，$n\in\NN_+$，其中 $p\leqslant1$，证明 $\{S_n\}$ 发散。
  \item 天文学中的 Kepler（开普勒）方程 $x-q\sin x=a$（$0<q<1$）是一个超越方程，没有求根公式（见 [15] 的 22 和 72 页），求近似解的一个方法是通过迭代。取定 $x_1$，然后用递推公式 $x_{n+1}=q\sin x_n+a$，$n\in\NN_+$。证明这个方法的正确性。
...
\begin{enumerate}
  ...
  \item 用覆盖定理证明闭区间套定理。
  \item 用覆盖定理证明凝聚定理。
  \item 试对例题 3.5.2 的证明举出两个具体例子，即 (1) 数集 $A$ 无上界；(2) $A$ 有上界，且有 $b<\xi=\sup A$ 和 $\xi\notin A$。
\end{enumerate}
\end{xexercise}
...
\begin{enumerate}
  ...
  \end{enumerate}
  \item 若 $x_n\geqslant y_n$，$n\in\NN_+$，证明：
  \[
  ...
  \]
  \item 对 $\{x_n\}$，记 $S_n=\frac1n\sum_{k=1}^{n}x_k$，$n\in\NN_+$，证明：
  \[
    \underline{\lim}_{n\to\infty}x_n
  ...
  \]
  \item 根据极限点的定义直接证明：每个数列的极限点集中必有最大数和最小数（不要利用命题 3.6.2 和 3.6.3 的现成结论）。
  \item 证明在例题 3.6.2 中所用到的关于上、下极限的公式。
  \item 证明公式 (3.2)。
  \item 对上极限写出与命题 3.6.3 对应的结论，并作出证明。
  \item 证明公式 (3.5)。
  \item 证明命题 3.6.5。
\end{enumerate}
\end{xexercise}
...
\begin{enumerate}
  ...
  \item 证明：数列收敛的充分必要条件是存在一个数 $a$，使数列的每个子列有收敛于 $a$ 的子列。
  \item 证明：在有界闭区间上的无界函数一定在这个区间的某一点的每一个邻域上无界。又问：在开区间上的无界函数是否有与此类似的性质？
  \item 设函数 $f$ 在区间 $(a,b)$ 上定义，对区间 $(a,b)$ 的每一个点 $\xi$，存在 $\delta>0$，当 $x\in(\xi-\delta,\xi+\delta)\cap(a,b)$ 时，如 $x<\xi$，则 $f(x)<f(\xi)$；如 $x>\xi$，则 $f(x)>f(\xi)$。证明：函数 $f$ 在 $(a,b)$ 上严格单调增加。
  \item 试用上、下极限的工具证明第二章 2.4.1 小节中的 Stolz 定理。

  （参考 3.6.4 小节的题 3。）

  \item 设 $\{x_n\},\{y_n\}$ 是非负数列，在以下乘积均有意义时证明：
  \[
    \underline{\lim}_{n\to\infty}x_n\,
    \underline{\lim}_{n\to\infty}y_n
  ...
  \[
    ...
    \overline{\lim}_{n\to\infty}y_n.
  \]
  \item 设 $\{x_n\}$ 为正数列。用上、下极限证明：若 $\lim_{n\to\infty}\frac{x_{n+1}}{x_n}=l$，则 $\lim_{n\to\infty}\sqrt[n]{x_n}=l$。
  \item 若对于数列 $\{a_n\}$ 的每个子列 $\{a_{n_k}\}$ 都有
  \[
...
\begin{xreferenceproblem}
\begin{enumerate}
  \item 从确界存在定理出发，证明：对于 $\RR$ 中的任何两个正数 $a,b$，存在正整数 $n$，使得 $na>b$。（这个结论常称为 Archimedes 公理或原理。）
  \item 设有两个非空实数集 $A$ 和 $B$，满足条件：(1) $\RR=A\cup B$；(2) 在 $A$ 中的每一个数都小于 $B$ 中的每一个数，证明：或者 $A$ 有最大数而 $B$ 无最小数，或者 $B$ 有最小数而 $A$ 无最大数。（这就是 Dedekind（戴德金）的连续性定理或公理，它与实数系的每一个基本定理等价。）
  \item 证明：将实数 $\RR$ 分成两个非空集合 $A$ 和 $B$，则或者 $A$ 中有数列收敛于 $B$ 中的点，或者 $B$ 中有数列收敛于 $A$ 中的点。（这个结论称为实数的连通性，它与实数系的每一个基本定理等价。）
  \item 试用压缩映射原理证明数列
  \[
    \sqrt7,\quad
    \sqrt{7-\sqrt7},\quad
  ...
  \item 设有界数列 $\{x_n\}$ 具有如下性质：对于每个数列 $\{y_n\}$，成立
  \[
    \overline{\lim}_{n\to\infty}(x_n+y_n)
  ...
  \item \begin{enumerate}[label=(\arabic*)]
    ...
    \]
  \end{enumerate}
  \item 设 $y_n=px_n+qx_{n+1}$，$n\in\NN_+$，其中 $\abs p<\abs q$。证明：若 $\{y_n\}$ 收敛，则 $\{x_n\}$ 也收敛。
  \item 设 $\{x_n\}$ 有界，且 $\lim_{n\to\infty}(x_{2n}+2x_n)=A$。证明 $\{x_n\}$ 收敛，并求其极限。
  \item 设 $x_n=\sin n$，$n\in\NN_+$，证明数列 $\{x_n\}$ 的极限点集合为 $[-1,1]$。
  \item 设 $\{x_n\}$ 有界，且 $\lim_{n\to\infty}(x_{n+1}-x_n)=0$。将 $\{x_n\}$ 的下极限和上极限分别记为 $l$ 和 $L$，证明：在区间 $[l,L]$ 中的每一个点都是数列 $\{x_n\}$ 的极限点。
