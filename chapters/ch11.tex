...
\begin{xexercise}
\begin{enumerate}
  \item 设椭圆方程为$Ax^2+2Bxy+Cy^2=1$，其中$A>0,\Delta=AC-B^2>0$。试用例题11.1.1中的第三种方法（即公式(11.1)）证明：由该椭圆所围的面积等于$\pi/\sqrt\Delta$。
  \item 试以公式(11.1)为出发点，推导出极坐标下的扇形面积计算公式(11.2)。
  \item 已知三个半径为$r$的圆，其中每个圆的圆周都通过另外两个圆的圆心，求三个圆公共部分的面积。

  （本题有不用微积分的初等解法。）
  \item 周长一定的等腰三角形，腰与底的比例为多少时，它绕底边旋转所得的旋转体体积最大？
  \item 半轴长为$a$和$b$的一个椭圆在曲线$y=c\sin(x/a)$上进行无滑动的滚动。问$a,b,c$之间的关系怎样时，椭圆在曲线上滚动了曲线的一个周期时，它正好转了一周？
  \item 在单位圆周上任意取一段位于第一象限且长度为$s$的弧，设位于该弧下方、$x$轴上方的曲边梯形的面积为$A$，而位于该弧左侧、$y$轴右侧的曲边梯形的面积为$B$。证明：$A+B$只依赖于弧的长度$s$，而与弧的位置无关。
  \item 试求由抛物线$y^2=2x$与过其焦点的弦所围的图形面积的最小值。
  \item 至少用两种方法计算下列三个圆的公共部分的面积：
  \[
    x^2+y^2\le4,\qquad (x-2)^2+y^2\le4,\qquad x^2+(y-2)^2\le4.
  \]
  \item 求椭圆柱$x^2/16+y^2/100\le1$夹在平面$z=0,y=2z$之间部分的体积。
  \item 求圆柱面$x^2+y^2=a^2$与$x^2+z^2=a^2$所围立体区域的体积。
...
\begin{xexercise}
\begin{enumerate}
  \item 设$f$在$[a,b]$上单调增加，证明：对每个$c\in(a,b)$，函数$F(x)=\int_c^xf(t)\,\dd t$为$[a,b]$上的下凸函数。
  \item 设$f$在$[0,+\infty)$上是下凸函数，证明：函数$F(x)=\frac1x\int_0^xf(t)\,\dd t$是$(0,+\infty)$上的下凸函数。
  \item 设$f$于$[0,1]$上为非负的上凸函数，证明：$\int_0^12f(x)\,\dd x\ge\max_{x\in[0,1]}\{f(x)\}$。
  \item 设$f$在$[a,b]$上为上凸可微函数，$f(a)=f(b)=0,f'(a)=\alpha>0,f'(b)=\beta<0$，证明：
  \[
    0\le\int_a^bf(x)\,\dd x\le\frac12\alpha\beta\cdot\frac{(b-a)^2}{\beta-\alpha}.
  \]
  \item 设$f\in C^1[0,2],f(0)=f(2)=1,\abs{f'(x)}\le1$，证明：$\abs{\int_0^2f(x)\,\dd x}\ge1$。
  \item 已知函数$f\in C[a,b]$，且$f(x)$处处大于0，证明：
  \[
    \int_a^bf(x)\,\dd x\int_a^b\frac1{f(x)}\,\dd x\ge(b-a)^2.
  \]
  \item 已知非负函数$f\in R[a,b],\int_a^bf(x)\,\dd x=1,k$为实数，证明：
  \[
    \left(\int_a^bf(x)\cos kx\,\dd x\right)^2+
    \left(\int_a^bf(x)\sin kx\,\dd x\right)^2\le1.
  \]
  \item 设$f\in C^1[a,b]$且$f(a)=0$，证明比例题11.2.3更强的不等式：
  \[
    \int_a^bf^2(x)\,\dd x\le\frac{(b-a)^2}{2}\int_a^b(f'(x))^2\,\dd x
    -\frac12\int_a^b(f'(x))^2(x-a)^2\,\dd x.
  \]
  \item 设$f$在$[0,1]$上可微且当$x\in(0,1)$时，$0\le f'(x)\le1,f(0)=0$。证明：
  \[
    \left(\int_0^1f(x)\,\dd x\right)^2\ge\int_0^1f^3(x)\,\dd x,
  \]
  且仅当$f(x)=0$或$f(x)=x$时成立等号。
  \item (1)试用Young不等式证明：当$a,b\ge1$时成立$ab\le\ee^{a-1}+b\ln b$；
...
\begin{enumerate}
  ...
  \]
  \item 设$f$在$[a,b]$上可微，$f(a)=f(b)=0,\abs{f'(x)}\le M$，证明：
  \[
    \abs{\int_a^bf(x)\,\dd x}\le\frac M4(b-a)^2.
  \]
  \item 设$f$在$[a,b]$上二阶可微，$f(a)=f(b)=0,\abs{f''(x)}\le M$，证明：
  \[
    \abs{\int_a^bf(x)\,\dd x}\le\frac M{12}(b-a)^3.
  \]
  \item （矩形公式）设$f\in C^2[a,b]$，证明：存在$\xi\in(a,b)$，使成立
  \[
    \int_a^bf(x)\,\dd x-(b-a)f\left(\frac{a+b}{2}\right)=\frac{f''(\xi)(b-a)^3}{24}.
  \]
  \item 设$f$于$[-1,1]$上可微，且有$a\in(0,1)$，使得$\int_{-a}^af(x)\,\dd x=0$，证明：
  \[
    \abs{\int_{-1}^1f(x)\,\dd x}\le M(1-a^2).
  \]
  % CHECK-SOURCE: 原书此题右端使用 M，但本题题设未见 M 的定义。
  \item 设$f$于$[0,1]$上可微，$\abs{f'(x)}\le M$，证明：
  \[
    \abs{\int_0^1f(x)\,\dd x-\frac1n\sum_{k=1}^nf\left(\frac kn\right)}\le\frac M{2n}.
  \]
  \item 设$f$在区间$[0,1]$上可微，且$f'\in R[0,1]$，对正整数$n$定义
  \[
    A_n=\int_0^1f(x)\,\dd x-\frac1n\sum_{k=1}^nf\left(\frac kn\right),
  \]
  证明：$\displaystyle\lim_{n\to\infty}nA_n=\frac12(f(0)-f(1))$。
  \item 设$f$在区间$[0,1]$上二阶可微，且$f''\in R[0,1]$，对正整数$n$定义
  \[
...
\begin{enumerate}
  ...
  \item 证明对于区间$(2/3,1)$中的数$A$，存在$N$使$n>N$时，成立
  \[
    \sqrt1+\sqrt2+\cdots+\sqrt n<An^{3/2},
  \]
  并与11.3.3小节的题5在方法和结果上进行比较。
  \item 设$f(x)\in C[0,1]$，$f(x)$处处大于0，求极限
  \[
  ...
  \item 设$A_n=\frac1{n+1}+\frac1{n+2}+\cdots+\frac1{2n}$，求$\displaystyle\lim_{n\to\infty}n(\ln2-A_n)$。
  \item 设$B_n=\frac2{2n+1}+\frac2{2n+3}+\cdots+\frac2{4n-1}$，求$\displaystyle\lim_{n\to\infty}n^2(\ln2-B_n)$。
  \item 求$\displaystyle\lim_{n\to\infty}\sqrt n\int_{-1}^1(1-x^2)^n\,\dd x$。
  \item 试从Stirling公式（命题11.4.2）的证明中推导出
  \[
  ...
  \item 试写出$(2n)!!,(2n-1)!!$和$C_{2n}^n$的渐近公式。
  \item 利用Stirling公式计算下列极限：
  \begin{enumerate}[label=(\arabic*)]
...
\begin{enumerate}
  \item 设$f\in C^1[0,1]$，且$f(0)=0,f(1)=1$，证明：
  \[
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \]
  \end{enumerate}
  \item 设$f$在$[0,1]$上连续，且$0\le f(x)<1$，证明：
  \[
  ...
  \]
  \item 设$f^{(2n)}\in C[a,b]$，且$f^{(k)}(a)=f^{(k)}(b)=0,k=0,1,2,\ldots,n-1$，证明：
  \[
    \abs{\int_a^bf(x)\,\dd x}
    \le\frac{(n!)^2(b-a)^{2n+1}}{(2n)!(2n+1)!}
    \max_{a\le x\le b}\{\abs{f^{(2n)}(x)}\}.
  \]
  \item 设函数$f,g\in C[a,b]$，且$f(x)\not\equiv0,g(x)$处处大于0。记
  \[
  ...
  \[
    ...
    =\ee^{-a^4/2}.
  \]
  \item 设$f$是$[1,+\infty)$上的非负单调减少函数，令
  \[
  ...
  \item 计算积分$\int_0^1\sin x^2\,\dd x$，使得误差不超过0.001。
  \item 证明：函数$F(x)=\int_0^x\sin\frac1t\,\dd t$在区间$(0,1]$上有无穷多个零点。
  \item 设$f$在$[a,b]$上可导，$f'$单调增加且有$f'(x)\ge m>0$，证明：
  \[
    \abs{\int_a^b\cos f(x)\,\dd x}\le\frac2m.
  \]
  \item 设$f$在$[a,b]$上连续可微，且满足$f'(x)\ge m>0,\abs{f(x)}\le\pi$，证明：
  \[
...
\begin{xreferenceproblem}
\begin{enumerate}
  \item 曲线$K$的极坐标方程为$\rho=\rho(\theta),0\le\theta\le\pi,\rho\in C[0,\pi]$，且已知$K$上任何两点之间的距离不超过1，证明：由曲线$K$与射线$\theta=0,\theta=\pi$围成的扇形面积
  \[
    S=\frac12\int_0^\pi\rho^2(\theta)\,\dd\theta\le\frac\pi4.
  \]
  （由此可见，直径不超过1的图形面积最多为$\pi/4$。）

  \item 设$f\in C[a,b]$，且处处大于0。记$f_{kn}=f(a+kh_n),h_n=(b-a)/n,k=1,2,\ldots,n$。证明：
  \[
  ...
  \item 在命题11.2.3（即Young不等式）中的可微条件可以去掉，此外还可以得到另一个方向的不等式。设$f\in C[0,+\infty)$，严格单调增加，且$f(0)=0$，记其反函数为$g(y)$。对$a,b>0$，证明下列不等式并解释其几何意义：
  \[
    ab\le\int_0^af(x)\,\dd x+\int_0^bg(y)\,\dd y
  ...
  \item （Bellman-Gronwall不等式）设当$x\ge0$时$f(x),g(x)$为非负连续函数，且有
  \[
  ...
  \item 设$f\in C^2[0,1],f(0)=f(1)=0$，$f$在$(0,1)$中无零点，证明：
  \[
    \int_0^1\abs{\frac{f''(x)}{f(x)}}\,\dd x\ge4,
  \]
  且其中4是最佳下界。

  \item 设$f$在$[0,1]$上可积，且有$0<m\le f(x)\le M$，则有
  \[
  ...
  \item 设非常值函数$f$在区间$[a,b]$上可微，且$f(a)=f(b)=0$，证明：在$[a,b]$内至少存在一点$\xi$，使
  \[
    \abs{f'(\xi)}>\frac4{(b-a)^2}\int_a^b\abs{f(x)}\,\dd x.
  \]

  \item 设$f\in C^2[0,1],f(0)=f(1)=f'(0)=0,f'(1)=1$，证明：
  \[
    \int_0^1(f''(x))^2\,\dd x\ge4,
  \]
  且其中成立等号当且仅当$f(x)=x^3-x^2$。

  \item 设函数$f$在区间$[a,b]$上处处大于0，且对于$L>0$满足Lipschitz条件
  \[
  ...
  \item 先用微分学或其他方法证明：当$0<x<1$时，成立不等式
  \[
