...
\begin{xexercise}
\begin{enumerate}
  \item 设 $f$ 在 $[0,+\infty)$ 上非负连续，且积分 $\int_0^{+\infty}f=0$，证明：$f\equiv0$。
  \item 如果广义积分 $\int_a^b f^2$ 收敛，则称 $f$ 在 $[a,b]$ 上平方可积。分无穷限积分与无界积分两种情况讨论广义积分的平方可积性与绝对可积性之间的关系。
  \item 设 $f\in C(0,1]$，$f(0+)=+\infty$，定义函数 $f_n(x)=\min\{f(x),n\}$，$0<x\leq1$，$n\in\NN_+$。证明：广义积分 $\int_0^1f$ 收敛的充分必要条件是存在有限极限
  \[
    \lim_{n\to\infty}\int_0^1 f_n(x)\,\dd x.
  \]
  \item 设 $f$ 在 $(-\infty,+\infty)$ 上内闭可积，$\int_{-\infty}^{+\infty}f^2$ 收敛，证明：对于任何实数 $a$，下列广义积分也收敛：
  \[
    \int_{-\infty}^{+\infty}\abs{f(x)f(x+a)}\,\dd x.
  \]
  \item 设 $f$ 在 $(-\infty,+\infty)$ 上内闭可积，并且 $\lim_{x\to+\infty}f(x)=A$，$\lim_{x\to-\infty}f(x)=B$ 都存在且有限。证明：对任意实数 $a>0$，广义积分
  \[
    \int_{-\infty}^{+\infty}[f(x+a)-f(x)]\,\dd x
  \]
  收敛，并求它的值。
  \item 找出以下广义积分计算中的错误，说明理由，并计算其主值。在积分
  \[
...
\begin{xexercise}
\begin{enumerate}
  \item 讨论下列广义积分的敛散性，若是收敛，还要讨论是条件收敛还是绝对收敛：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 对于以下含有参数的广义积分，确定出使积分绝对收敛、条件收敛和发散的参数范围：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 设 $a_1,\ldots,a_n$ 为互不相同的实数，$p_1,\ldots,p_n>0$，讨论广义积分
  \[
  ...
  \item 设 $f'\in C[0,1]$ 且 $f'(x)$ 处处大于 0，证明：广义积分
  \[
    \int_0^1\frac{f(x)-f(0)}{x^p}\,\dd x
  \]
  在 $p<2$ 时收敛，$p\geq2$ 时发散。

  \item 设 $\int_a^{+\infty}f$ 为条件收敛，证明：
  \begin{enumerate}[label=(\arabic*)]
    \item 广义积分 $\int_a^{+\infty}(\abs{f}\pm f)$ 发散；
    \item $\displaystyle\lim_{x\to+\infty}\frac{\int_a^x(\abs{f}+f)}{\int_a^x(\abs{f}-f)}=1$。
  \end{enumerate}

  \item 设 $f\in C^1[a,+\infty)$ 单调，且 $f(+\infty)=0$，证明：$\int_a^{+\infty}f'(x)\sin^2x\,\dd x$ 收敛。

  \item 设 $f,g\in C^1[a,+\infty)$，$f'(x)$ 非负，$f(+\infty)=0$，且 $g(x)$ 在 $[a,+\infty)$ 上有界，证明：$\int_a^{+\infty}f(x)g'(x)\,\dd x$ 收敛。
\end{enumerate}
\end{xexercise}
...
\begin{enumerate}
  ...
  \item 利用 Euler 积分（例题 12.3.4）计算下列积分：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 利用 Froullani 积分（例题 12.3.5 及其注）计算下列积分（$a,b>0$）：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 利用 Dirichlet 积分（例题 12.3.6）计算下列积分：
  \begin{enumerate}[label=(\arabic*)]
...
\begin{xremark}
可以进一步证明存在数列 $\{x_n\}\subset[a,+\infty)$，满足条件 $\lim_{n\to\infty}x_n=+\infty$，$\lim_{n\to\infty}x_nf(x_n)=0$。见下面的练习题 6。
\end{xremark}
...
\begin{xexercise}
\begin{enumerate}
  \item 设 $f$ 于 $[a,+\infty)$ 上可导，$f'$ 内闭可积，且广义积分 $\int_a^{+\infty}f(x)\,\dd x$ 和 $\int_a^{+\infty}f'(x)\,\dd x$ 都收敛，证明：$\lim_{x\to+\infty}f(x)=0$。

  \item 设函数 $f(x)$ 在 $[a,+\infty)$ 上有有界的导函数且无穷限积分 $\int_a^{+\infty}f(x)\,\dd x$ 收敛，证明：$\lim_{x\to+\infty}f(x)=0$。

  \item 举例说明例题 12.4.2 之逆不成立，也就是说，当函数 $f$ 在 $[a,+\infty)$ 上单调，且满足条件 $\lim_{x\to+\infty}xf(x)=0$ 时，广义积分 $\int_a^{+\infty}f(x)\,\dd x$ 仍可能发散。

  \item 若 $\int_a^{+\infty}f(x)\,\dd x$ 收敛，且 $xf(x)$ 单调，证明：$\lim_{x\to+\infty}xf(x)\ln x=0$。

  \item 设函数 $f$ 在 $[a,+\infty)$ 上可微且无穷限积分 $\int_a^{+\infty}f(x)\,\dd x$ 收敛，证明：存在数列 $\{x_n\}$，使 $\lim_{n\to\infty}x_n=+\infty$，$\lim_{n\to\infty}f'(x_n)=0$。
...
\begin{enumerate}
  ...
  \item 设 $f$ 在 $(-\infty,+\infty)$ 上内闭可积，$p\geq1$，且 $\abs{f}^p$ 在 $(-\infty,+\infty)$ 上可积，证明：
  \[
    \lim_{h\to0}\int_{-\infty}^{+\infty}\abs{f(x+h)-f(x)}^p\,\dd x=0.
  \]

  \item 设 $f,g$ 在 $(-\infty,+\infty)$ 上内闭可积，$p>1$，且 $\abs{f}^p$，$\abs{g}^{p/(p-1)}$ 在 $(-\infty,+\infty)$ 上可积，证明：函数
  \[
    I(t)=\int_{-\infty}^{+\infty}f(x+t)g(x)\,\dd x
  \]
  在 $(-\infty,+\infty)$ 上连续。

  \item 设 $f\in C^1[a,+\infty)$，单调减少，且 $f(+\infty)=0$，证明：广义积分 $\int_a^{+\infty}f(x)\,\dd x$ 收敛的充分必要条件是 $\int_a^{+\infty}xf'(x)\,\dd x$ 收敛。

  \item 设 $f$ 在 $[a,+\infty)$ 上为内闭可积的正函数，且有 $\lim_{x\to+\infty}\frac{\ln f(x)}{\ln x}=p$，则当 $-\infty\leq p<-1$ 时，积分 $\int_a^{+\infty}f(x)\,\dd x$ 收敛，而当 $-1<p\leq+\infty$ 时，积分 $\int_a^{+\infty}f(x)\,\dd x$ 发散。
  ...
  \item 讨论以下带有参数的广义积分的敛散性，确定使得积分绝对收敛、条件收敛和发散的参数范围：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 设 $f$ 在 $[a,+\infty)$ 上单调有界，广义积分 $\int_a^{+\infty}f(x)\sin px\,\dd x$ 在 $p>0$ 时收敛，证明：
  \[
    \lim_{p\to+\infty}\int_a^{+\infty}f(x)\sin px\,\dd x=0.
  \]

  \item 设 $f\in C[0,+\infty)$，广义积分 $\int_0^{+\infty}\varphi(x)\,\dd x$ 绝对收敛，证明：
  \[
    \lim_{n\to\infty}\int_0^{\sqrt n}f\left(\frac xn\right)\varphi(x)\,\dd x
    =f(0)\int_0^{+\infty}\varphi(x)\,\dd x.
  \]

  \item 设 $f$ 在 $(-\infty,+\infty)$ 上绝对可积，证明：
  \begin{enumerate}[label=(\arabic*)]
  ...
  \item 在常义积分的积分第二中值定理的基础上，证明广义积分第二中值定理：设广义积分 $\int_a^b g(x)\,\dd x$ 收敛（奇点为 $a$ 或 $b$，或者 $a$ 和 $b$ 都是奇点），如果 $f$ 在 $(a,b)$ 上单调有界，则存在 $\xi\in[a,b]$，使
  \[
    \int_a^b f(x)g(x)\,\dd x
    =f(a+)\int_a^\xi g(x)\,\dd x+f(b-)\int_\xi^b g(x)\,\dd x.
  \]

  \item 设广义积分 $\int_1^{+\infty}f(x)\,\dd x$ 收敛。证明：存在 $\xi\in(1,+\infty)$，使得
  \[
    \int_1^{+\infty}x^{-1}f(x)\,\dd x=\int_1^\xi f(x)\,\dd x.
  \]

  \item 设 $a>0$，$f$ 在 $[a,+\infty)$ 上平方可积，证明：积分 $\int_a^{+\infty}\frac{f(x)}x\,\dd x$ 收敛。
...
\begin{enumerate}
  ...
  \item \textbf{（Gordon 不等式）}\quad 证明：函数
  \[
  ...
  \item 设 $f\in C[0,+\infty)$ 且平方可积，令 $g(x)=\int_0^x f(t)\,\dd t$，证明：$g(x)/x$ 在 $[0,+\infty)$ 上平方可积，且成立
  \[
    \int_0^{+\infty}\frac{g^2(x)}{x^2}\,\dd x\leq4\int_0^{+\infty}f^2(x)\,\dd x.
  \]

  \item 设 $f$ 在 $[0,+\infty)$ 上二阶可微，$f$ 和 $f''$ 在这个区间上均平方可积，证明：$f'$ 在这个区间上也平方可积。

  \item 设 $f\in C^1[0,+\infty)$，且 $xf(x)$ 和 $f'(x)$ 在这个区间上均平方可积，证明：
  \begin{enumerate}[label=(\arabic*)]
    \item $f$ 也在这个区间上平方可积；
    \item 成立不等式
    \[
...
% SOURCE-ERR: 原书作 $1/x_k$ 项；由部分分式分解及紧随其后的 $A_k=P(x_k)/Q'(x_k)$ 可确定应为 $1/(x-x_k)$ 项。
  ...
  \item 设 $f$ 是 $(-\infty,+\infty)$ 上的非负函数，且满足以下条件：
  \[
