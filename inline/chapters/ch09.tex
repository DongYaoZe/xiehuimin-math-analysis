...
\begin{xthought}[3]
在不定积分公式 $\int \operatorname{sgn}x\,\dd x=\abs{x}+C$ 和 $\int\dfrac1x\,\dd x=\ln\abs{x}+C$ 中为什么会出现绝对值号？
\end{xthought}
\begin{xsolution}
在公式
  \[
...
用配对积分法计算下列不定积分。
  \begin{enumerate}[label=(\arabic*)]
    ...
    令
    \[
      I=\int\frac{\dd x}{1+x^2+x^4},
      \qquad
      J=\int\frac{x^2\,\dd x}{1+x^2+x^4}.
    \]
    对 $x\ne0$，分别配对可得
    \begin{align*}
      I+J
      &=\int\frac{1+x^2}{1+x^2+x^4}\,\dd x
       =\int\frac{\dd(x-1/x)}{(x-1/x)^2+3}\\
      &=\frac1{\sqrt3}\arctan\frac{x^2-1}{\sqrt3x}+C_1,
    \end{align*}
    以及
    \begin{align*}
      J-I
      &=\int\frac{x^2-1}{1+x^2+x^4}\,\dd x
       =\int\frac{\dd(x+1/x)}{(x+1/x)^2-1}\\
      &=\frac12\ln\frac{x^2-x+1}{x^2+x+1}+C_2.
    \end{align*}
    解这两个线性关系即可得到 $I$。将反正切的分支常数并入 $C$，并把结果写成在 $x=0$ 处也有定义的形式，得
    \[
      \boxed{
    ...
    令
    \[
      I=\int\frac{\ee^x\,\dd x}{\ee^x+\ee^{-x}},
      \qquad
      J=\int\frac{\ee^{-x}\,\dd x}{\ee^x+\ee^{-x}}.
    \]
    则
    \[
      I+J=x+C_1,
      \qquad
      I-J=\ln(\ee^x+\ee^{-x})+C_2.
    \]
    解得
    \[
      \boxed{I=\frac12x+\frac12\ln(\ee^x+\ee^{-x})+C}
      =\frac12\ln(1+\ee^{2x})+C.
    \]
    ...
    令
    \[
      I=\int\frac{\dd x}{1+x^3},
      \qquad
      J=\int\frac{x\,\dd x}{1+x^3},
      \qquad
      K=\int\frac{x^2\,\dd x}{1+x^3}
       =\frac13\ln\abs{1+x^3}+C.
    \]
    配对后有
    \[
      K-J+I=\int\frac{\dd x}{x+1}=\ln\abs{x+1}+C_1,
    \]
    以及
    \[
      I+J=\int\frac{\dd x}{x^2-x+1}
      =\frac2{\sqrt3}\arctan\frac{2x-1}{\sqrt3}+C_2.
    \]
    将两式相加并消去 $J$，得到
    \[
      \boxed{
...
\end{xexercise}
\begin{xsolution}
通过两个配对积分求 $\int\cos^4x\,\dd x$ 与 $\int\sin^4x\,\dd x$。
...
对每个正整数 $n$，定义
  ...
  \]
  上述公式在 $c\ne0$ 且 $cx+b>0$ 的任一连通区间上使用。若 $c=0$，实变量下要使被积函数有定义必须有 $b>0$，此时无需递推：当 $a\ne0$ 时
  \[
    I_n=\frac{(ax+b)^{n+1}}{a(n+1)\sqrt b}+C,
  \]
  而当 $a=0$ 时
  \[
    I_n=b^{n-1/2}x+C.
...
设 $Q$ 为 $n$ 次多项式，且具有 $n$ 个相异实根 $x_i$，$i=1,2,\ldots,n$。又设 $P$ 是与 $Q$ 不可约的 $m$ 次多项式，且 $m<n$，证明：
  \[
...
\end{xreferenceproblem}
\begin{xsolution}
设 $Q$ 为 $n$ 次多项式，具有 $n$ 个相异实根 $x_i$；$P$ 与 $Q$ 不可约，且 $\deg P=m<n$。证明
  \[
    \int\frac{P(x)}{Q(x)}\,\dd x
    =\sum_{i=1}^n\frac{P(x_i)}{Q'(x_i)}\ln\abs{x-x_i}+C.
  \]
...
求 $\int f(x)\,\dd x$，其中 $f(x)$ 为 $x$ 到离其最近的整数的距离。
\end{xreferenceproblem}
\begin{xsolution}
\textbf{解}\quad 令
  \[
...
Liouville 在 19 世纪 30 年代对于初等函数的不定积分在什么条件下是初等函数进行过深入的研究（参见 [55]），他得到的一个结果是：

  \begin{xtheorem}
  设 $f,g$ 为有理函数，$g$ 不是常值函数。如果
  \[
    \int f(x)\ee^{g(x)}\,\dd x
  \]
  是初等函数，则存在有理函数 $h$，使得
