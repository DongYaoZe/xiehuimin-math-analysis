...
\paragraph{题 1}
\begin{xsolution}
这里的“不宜”并不都表示 L'Hospital 法则不能使用，而是指直接化简更自然。
\begin{enumerate}[label=(\arabic*)]
  \item 当 $x\to1$ 时原式不是不定式，直接代入得 $1/2$。
  \item 分子、分母同除以 $x$，得到
  \[
    \frac{1+\sin x/x}{1-\sin x/x}\to1.
  \]
  若求导，得到 $(1+\cos x)/(1-\cos x)$，反而失去极限，故不宜使用 L'Hospital 法则。
  \item 分子、分母同除以 $\ee^x$，得到
  \[
    \frac{1+\ee^{-2x}}{1-\ee^{-2x}}\to1.
  \]
  L'Hospital 法则虽可用，但只是把问题改写成同样量级的指数式，没有必要。
  \item 因 $x\to+\infty$，直接除以 $x$：
  \[
    \frac{x}{\sqrt{1+x^2}}=\frac1{\sqrt{1+x^{-2}}}\to1.
  \]
  L'Hospital 法则也可用，但显然比这一化简繁琐。
\end{enumerate}
\end{xsolution}
...
\]
故 $\liminf_{x\to+\infty}f(x)/x\ge m>0$，矛盾。若 $m<0$，向 $-\infty$ 作同样比较得到 $\limsup_{x\to-\infty}f(x)/x\le m<0$，也矛盾。因此所有割线斜率均为零，$f$ 为常值函数。
\end{xsolution}
...
\]
所以 $f$ 单调增加且有上界，极限存在，并且
\[
  \lim_{x\to+\infty}f(x)\le1+\frac\pi4.
\]
实际上对每个有限 $x>1$ 上式严格，且由 $f(x)>1$ 可在任意尾区间保持严格余量，故极限严格小于 $1+\pi/4$。
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \]
  因而在 $(0,1)$ 严格增加，从 $y(0)=\ee$ 增到 $+\infty$；在 $(1,\sqrt3)$ 从 $0$ 墓到极大值
  \[
...
\paragraph{题 4}
% CHECK-SOLUTION: 当前 ch08 只交叉引用“2.6.3 小节的题 9”，未给出该算法的递推公式；按用户要求只读本章，无法可靠重建并证明其三阶收敛性。
\begin{xsolution}
本题所需的具体迭代公式不在本章题面中。为避免猜测算法，保留待由整合者在允许读取 2.6.3 题 9 后补全。
\end{xsolution}
...
\[
  ...
  \end{cases}
\]
在 $(-1,1)$ 内从不取值 3。因此保留为待勘误题。
\end{xsolution}

\paragraph{第一组参考题 18}
\begin{xsolution}
反设 $g(x)$ 在 $(a,b)$ 内无零点。由连续性，$g$ 在整个 $[a,b]$ 上除端点可能性外保持非零；且若端点有零点，结合 Wronski 条件与 $f(a)=f(b)=0$ 可先在内缩区间作以下论证并取极限。直接在 $g\ne0$ 的情形令
\[
  h(x)=\frac{f(x)}{g(x)}.
\]
由 $f(a)=f(b)=0$ 得 $h(a)=h(b)=0$，Rolle 定理给出某 $\xi\in(a,b)$ 使 $h'(\xi)=0$。然而
\[
  h'(x)=\frac{f'g-fg'}{g^2}=-\frac{W(f,g)}{g^2}\ne0,
\]
矛盾。故 $g$ 必在 $(a,b)$ 中有零点。
\end{xsolution}
...
\]
故恰有一个二重根，其余两根保持为单根，于是共有 3 个互异实根。在四个尖点 $(\pm1,0),(0,\pm1)$，例如 $(1,0)$ 时
\[
