...
\paragraph{题 14}
\begin{xsolution}
设有理分式函数 $R(x)=P(x)/Q(x)$ 是周期函数，周期为 $T>0$。

若 $Q$ 有实零点 $x_0$，则周期性将使 $x_0+nT$（$n\in\ZZ$）都成为极点；但非零多项式 $Q$ 只有有限个零点，矛盾。因此若 $R$ 在其定义域上周期，只可能没有实极点。

再考察 $x\to+\infty$。若 $\deg P>\deg Q$，则 $\abs{R(x)}\to\infty$。但固定一个 $x_0$，有 $R(x_0+nT)=R(x_0)$，与此矛盾。故 $\deg P\leq\deg Q$，从而 $R(x)$ 在 $+\infty$ 有有限极限 $L$。对任意 $x$，
\[
  R(x)=R(x+nT)\to L,
\]
所以 $R(x)=L$。因此周期有理分式函数只能是常值函数，任何非常值有理分式函数都不可能是周期函数。
\end{xsolution}
...
\]
满足 $g$ 无极限而 $fg\to0$；而取 $f(x)\equiv1$、$g(x)=\sin(1/x)$ 时，乘积仍无极限。
\end{xsolution}
...
\paragraph{题 1}
\begin{xsolution}
(1) 令 $c=\ln a>0$。取整数 $m>k$。由指数函数的幂级数中各项非负，$x>0$ 时
\[
  \ee^{cx}\geq\frac{(cx)^m}{m!}.
\]
于是
\[
  0\leq\frac{x^k}{a^x}=\frac{x^k}{\ee^{cx}}
  \leq\frac{m!}{c^m x^{m-k}}\to0.
\]

(2) 令 $t=\ln x$，则 $t\to+\infty$，且
\[
  \frac{\ln x}{x^k}=\frac{t}{\ee^{kt}}\to0
\]
由 (1) 的同一估计（取幂次 1）即得。
...
\paragraph{题 11}
\begin{xsolution}
以 $f$ 在 $(a,b)$ 上单调增加为例。取一个严格增加且趋于 $b$ 的数列 $x_n\in(a,b)$。则 $f(x_n)$ 单调增加，故它要么收敛于有限数 $A$，要么趋于 $+\infty$。

若 $f(x_n)\to A<+\infty$，先说明 $f(x)\leq A$ 对每个 $x<b$ 成立：固定 $x$，充分大的 $n$ 有 $x_n>x$，故 $f(x)\leq f(x_n)\to A$。给定 $\varepsilon>0$，取 $N$ 使 $f(x_N)>A-\varepsilon$。当 $x_N<x<b$ 时
\[
  A-\varepsilon<f(x_N)\leq f(x)\leq A,
\]
所以 $f(x)\to A$（$x\to b^-$）。

若 $f(x_n)\to+\infty$，给定 $G>0$，取 $N$ 使 $f(x_N)>G$，则 $x_N<x<b$ 时 $f(x)>G$，故 $f(x)\to+\infty$。单调减少情形同理。这个证明也说明了单侧 Heine 原理中只考察适当的单调数列已足够。
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item
  \[
    a^{x^2}-1=\ee^{x^2\ln a}-1\sim(\ln a)x^2.
  \]
  若 $a\ne1$，它是二阶无穷小量；若 $a=1$，函数恒为 0，不具有题目定义下的有限正阶，可视为高于任意有限阶。
