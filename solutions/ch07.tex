...
% SOLVED-ITEM
\begin{xsolution}
可以构造一个例子，使闭区间套收敛到的点虽然满足 $f'(\xi)=0$，却不是极值点。

在 $[-1,1]$ 上定义
\[
  f(0)=0,
  \qquad
  f(x)=x^3\sin\bigl(\pi\log_2\abs{x}\bigr)\quad(x\neq0).
\]
由于 $\abs{f(x)}\leqslant\abs{x}^3$，有
\[
  f'(0)=\lim_{x\to0}\frac{f(x)-f(0)}x
  =\lim_{x\to0}x^2\sin\bigl(\pi\log_2\abs{x}\bigr)=0.
\]
而对每个正整数 $m$，
\[
  f(2^{-m})=f(-2^{-m})=0.
\]
因此在 Samuleson 构造中可以依次选取
\[
  [a_m,b_m]=[-2^{-m},2^{-m}],\qquad m=0,1,2,\ldots.
\]
这些区间满足 $f(a_m)=f(b_m)$，且
\[
  b_{m+1}-a_{m+1}=\frac12(b_m-a_m),
\]
所以闭区间套收敛到 $\xi=0$。

另一方面，$f$ 在 $0$ 的任意邻域内都取正值和负值。例如选取
\[
  \abs{x}=2^{-m-1/2}
  \quad\text{与}\quad
  \abs{x}=2^{-m-3/2},
\]
正弦因子符号相反。因此 $0$ 不是 $f$ 的极大值点或极小值点。这个例子说明 Samuleson 证明得到的 $\xi$ 不必是极值点。
\end{xsolution}
...
% SOLVED-ITEM
\begin{xsolution}
这种情形对导函数实际上不可能发生。若所谓“无穷大量但无确定符号”是指
\[
  \abs{f'(x)}\to+\infty\qquad(x\to a+)
\]
而 $f'(x)$ 在任意右邻域内都改变符号，则可取趋于 $a$ 的两列点 $x_n,y_n$，使
\[
  f'(x_n)>0,
  \qquad
  f'(y_n)<0.
\]
导函数具有 Darboux 介值性质，因此在 $x_n,y_n$ 之间必有 $z_n$ 使 $f'(z_n)=0$。又 $z_n\to a+$，这与 $\abs{f'(x)}\to+\infty$ 矛盾。

所以若导函数的绝对值趋于无穷大，则在充分靠近 $a$ 时符号必定固定，只可能有 $f'(x)\to+\infty$ 或 $f'(x)\to-\infty$ 两种情形；不存在题中所说的第三种情形。
\end{xsolution}
...
\]
因此所要求的等式成立。对 $x\geqslant0$，
\[
  0\leqslant\sqrt{x(x+1)}-x
  =\frac{x}{\sqrt{x(x+1)}+x}\leqslant\frac12,
\]
其中最后一个不等式等价于 $\sqrt{x^2+x}\leqslant x+1/2$。故
\[
...
% SOLVED-ITEM
\begin{xsolution}
若目的是用这两个计算来\emph{首次证明}基本极限，则不能这样做，因为会产生循环论证。

建立 $\sin x$、$\cos x$ 的 Taylor 公式，需要先知道它们在 $0$ 附近的各阶导数；而初等微分学中证明
\[
  (\sin x)'=\cos x,
  \qquad
  (\cos x)'=-\sin x
...
\]
等基本极限。因此若用已经依赖这些极限的 Taylor 展开反过来证明它们，逻辑上是循环的。

当然，如果在另一套独立理论中已经不依赖上述极限建立了 $\sin$、$\cos$ 的可微性及 Taylor 公式，那么这些展开式可以用来重新计算这些极限；问题只在于不能把它当作本课程逻辑链条中的第一次证明。
\end{xsolution}
...
因此按原文只能给出上述反例；若原题另有遗漏条件（例如还要求 $f(0)=0$），则需要在补全题意后另行证明，不能把缺失条件自行加入原题。
\end{xsolution}
...
\]
因此存在 $\delta>0$ 和 $M>0$，使对 $0<x\leqslant\delta$ 有
\[
...
\paragraph{第二组参考题 7}
% SOLVED-ITEM
% CHECK-SOLUTION: 原题同时写 f(a)=0 与 f(x)>0, \forall x\in[a,b]，两条件在 x=a 处直接矛盾；按原文不存在满足全部条件的函数，无法给出非空情形证明。
\begin{xsolution}
按原题文字，不存在满足全部假设的函数：由 $f(x)>0$ 对所有 $x\in[a,b]$，代入 $x=a$ 得 $f(a)>0$，与 $f(a)=0$ 矛盾。因此该命题按形式逻辑是空真命题，但没有可供构造 $x_1,x_2$ 的非空情形。需整合时回查原书是否本意为 $f(x)>0$ 对 $x\in(a,b]$ 或 $x\in(a,b)$。
\end{xsolution}
...
% SOLVED-ITEM
% CHECK-SOLUTION: 原题把函数定义在 [0,+\infty)，但不等式写作 \forall x\in[a,+\infty)，其中 a 未定义。若只要求从某个 a>0 起成立，结论一般为假，因此无法按原文可靠证明。
\begin{xsolution}
原题中的 $a$ 未定义，不能唯一确定题意。若把条件解释为“存在某个 $a>0$，且不等式只对 $x\geqslant a$ 成立”，结论甚至是假的。例如固定该 $a>0$，定义
\[
  f(x)=
  \begin{cases}
    x(a-x)^2,&0\leqslant x\leqslant a,\\
    0,&x\geqslant a.
  \end{cases}
\]
则 $f(0)=0$，且在 $x=a$ 左右一阶导数都为 $0$，所以 $f$ 在 $[0,+\infty)$ 可微；对所有 $x\geqslant a$，
\[
  \abs{f'(x)}=0\leqslant c\abs{f(x)}=0,
\]
但 $f$ 在 $(0,a)$ 并不恒为零。

若原题本意是不等式对所有 $x\in[0,+\infty)$ 成立，则可用标准 Bellman--Gronwall 论证证明 $f\equiv0$；但鉴于原文缺少这个关键信息，此处保留待核。
\end{xsolution}
...
以下证明其余各阶。固定 $h=1$。对 $j=1,\ldots,n-1$，Taylor 公式给出
\[
  f(x+j)-f(x)
  =\sum_{k=1}^{n-1}\frac{j^k}{k!}f^{(k)}(x)
