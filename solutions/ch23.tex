...
\qquad |t|<\frac12.
\]
只需检查 $0,1,2$ 三个可能的瑕点。对 $0<x\leqslant1$，
\[
x^t\leqslant x^{-1/2}.
\]
在 $x=1$ 与 $x=2$ 的邻域中，$x^t$ 关于 $|t|<1/2$ 一致有界，而分母分别只产生 $|x-1|^{-1/3}$、$|x-2|^{-1/3}$ 型奇性。因此可用三个可积函数分段控制被积函数绝对值，M-判别法给出在 $|t|<1/2$ 上\textbf{一致且绝对收敛}。
...
\]
所以可定义 $\varphi'(0)=0$。对 $x_0>0$，把上式令 $s=xu$ 可写成
\[
\varphi'(x)=\sqrt x\int_0^1\frac{f'(xu)}{\sqrt{1-u}}\,\dd u,
\]
由控制收敛定理知其关于 $x$ 连续。结合 $x=0$ 的极限，$\varphi\in C^1[0,1]$。
...
\textbf{(2)} 原书扫描页第二问写作 $g(t)$，但本题只有第一问定义了 $q(t)$；结合右端及题意，第二问应理解为对 $q(t)$ 的结论。记
\[
...
\]
先加收敛因子 $\ee^{-\varepsilon t}$，$\varepsilon>0$。由 Gamma 积分的 Laplace 形式，
\[
\int_0^{+\infty}t^{s-1}\ee^{-(\varepsilon-\ii)t}\,\dd t
=\Gamma(s)(\varepsilon-\ii)^{-s}.
\]
取实部，
\[
\int_0^{+\infty}t^{s-1}\ee^{-\varepsilon t}\cos t\,\dd t
=\Gamma(s)(1+\varepsilon^2)^{-s/2}
\cos\left(s\arctan\frac1\varepsilon\right).
\]
因为 $0<s<1$，原振荡积分由 Dirichlet 判别法收敛，并由 Abel 极限定理可令
$\varepsilon\to0^+$，得到
\[
...
\]
在 $x=1$ 的小闭邻域内，对数导数可逐项取极限，故
\[
\frac{\Gamma'(x)}{\Gamma(x)}
=\lim_{n\to\infty}
\left(\ln n-\sum_{k=0}^n\frac1{x+k}\right).
\]
令 $x=1$，利用 $\Gamma(1)=1$，
\[
\Gamma'(1)
=\lim_{n\to\infty}
\left(\ln n-\sum_{k=1}^{n+1}\frac1k\right).
\]
记 $H_n=\sum_{k=1}^n1/k$，则
\[
\begin{aligned}
-\Gamma'(1)
