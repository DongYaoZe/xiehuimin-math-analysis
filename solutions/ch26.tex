...
若 $f\not\equiv0$，分离变量得
\[
  \frac{f'(r)}{f(r)}=-\frac{3}{r}.
\]
积分后
\[
  \ln\abs{f(r)}=-3\ln r+C_0,
\]
故
\[
  \boxed{f(r)=\frac{C}{r^3}},\qquad r>0,
\]
其中 $C$ 为任意常数；$C=0$ 已包含零解。
\end{xsolution}
...
\subsection*{第一组参考题 3}
\begin{xsolution}
设 $L$ 位于半径为 $R>0$ 的球面上，取该球面上由 $L$ 围成的一片定向区域 $S$。记
\[
  \vect{X}=\nabla\times\vect{B}.
\]
题设给出
...
设 $\gamma(s)$ 是 $L$ 的弧长参数，$0\le s\le l$，并令
\[
  \vect{n}(s)=\frac{\gamma(s)}{R},
  \qquad
  \vect{\tau}(s)=\gamma'(s).
\]
对任意 $0<a<b$，把 $S$ 沿径向缩放到半径 $a$ 与 $b$ 之间，得到薄壳区域
\[
  \Omega_{a,b}=\left\{\frac{\rho}{R}p\ \middle|\ p\in S,\ a<\rho<b\right\}.
\]
在它的两个球面部分上，$\vect{X}$ 与法向量正交，所以通量为零。由 Gauss 公式及 $\nabla\cdot\vect{X}=0$，其侧面通量也必须为零。

侧面可参数化为
\[
  \Phi(s,\rho)=\frac{\rho}{R}\gamma(s).
\]
有
\[
  \Phi_\rho=\vect{n},
  \qquad
  \Phi_s=\frac{\rho}{R}\vect{\tau},
\]
故侧面的法向面积元为
\[
  \pm\frac{\rho}{R}(\vect{n}\times\vect{\tau})\,\dd s\,\dd\rho.
\]
又
\[
  \vect{X}(\Phi)
  =\frac1\rho\bigl(\vect{n}\times\vect{A}(\Phi)\bigr),
\]
且
\[
  (\vect{n}\times\vect{A})\cdot(\vect{n}\times\vect{\tau})
  =\vect{A}\cdot\vect{\tau},
\]
因为 $\vect{n}\cdot\vect{\tau}=0$。于是侧面通量为
\[
  \pm\frac1R\int_a^b\left[
  \oint_L\vect{A}\left(\frac{\rho}{R}\gamma(s)\right)
  \cdot\vect{\tau}(s)\,\dd s
  \right]\dd\rho=0.
\]
这对任意 $a<b$ 都成立，括号内的连续函数因而恒为零。取 $\rho=R$，得到
\[
...
先由散度公式
\[
  \nabla\cdot(u(x,y)(x,y))=xu_x+yu_y+2u
\]
得到
\begin{equation}
  I:=\iint_D(xu_x+yu_y)\,\dd x\,\dd y
  =\oint_{\partial D}u\,\dd s-2\iint_Du\,\dd x\,\dd y,
  \tag{1}
\end{equation}
因为在单位圆周上 $(x,y)\cdot\vect{n}=1$。

取
\[
  \psi(x,y)=\frac{1-r^2}{2}.
\]
则
\[
  \psi=0\quad\text{于 }\partial D,
  \qquad
  \frac{\partial\psi}{\partial n}=-1\quad\text{于 }\partial D,
  \qquad
  \Delta\psi=-2.
\]
对 $u,\psi$ 使用第二 Green 恒等式，
\begin{align*}
  \iint_D(\psi\Delta u-u\Delta\psi)\,\dd x\,\dd y
  &=\oint_{\partial D}\left(
    \psi\frac{\partial u}{\partial n}
    -u\frac{\partial\psi}{\partial n}
    \right)\dd s\\
  &=\oint_{\partial D}u\,\dd s.
\end{align*}
因此
\[
  \oint_{\partial D}u\,\dd s
  =\iint_D\psi\Delta u\,\dd x\,\dd y+2\iint_Du\,\dd x\,\dd y.
\]
代入 (1)，并利用 $\Delta u=\ee^{-r^2}$，得
\begin{align*}
  I
  &=\iint_D\frac{1-r^2}{2}\ee^{-r^2}\,\dd x\,\dd y\\
  &=\pi\int_0^1(1-r^2)\ee^{-r^2}r\,\dd r\\
  &=\frac{\pi}{2}\int_0^1(1-t)\ee^{-t}\,\dd t
  \qquad(t=r^2)\\
  &=\frac{\pi}{2\ee}.
\end{align*}
故
...
\subsection*{第二组参考题 1}
\begin{xsolution}
设连续函数 $u$ 在区域 $\Omega\subset\RR^n$ 中处处满足球面平均值公式。先证明 $u$ 无限次可微。

取任意 $x_0\in\Omega$ 以及充分小的 $\varepsilon>0$，使 $\overline{B_{2\varepsilon}(x_0)}\subset\Omega$。取径向对称的非负函数
\[
  \eta_\varepsilon\in C_c^\infty(B_\varepsilon(0)),
  \qquad
  \int\eta_\varepsilon(y)\,\dd y=1.
\]
对 $x\in B_\varepsilon(x_0)$，把卷积按球面分层，并在每一层应用平均值公式，得到
\[
  (u*\eta_\varepsilon)(x)
  =u(x)\int\eta_\varepsilon(y)\,\dd y=u(x).
\]
所以 $u$ 局部等于一个光滑卷积，因而 $u\in C^\infty(\Omega)$。

现在固定 $x\in\Omega$。对小的 $r>0$，在单位球面上作 Taylor 展开：
\[
  u(x+r\vect{\omega})
  =u(x)+r\nabla u(x)\cdot\vect{\omega}
  +\frac{r^2}{2}\vect{\omega}^{\mathsf T}H_u(x)\vect{\omega}
  +\littleo(r^2).
\]
对单位球面取平均，由对称性
\[
  \operatorname{avg}\omega_i=0,
  \qquad
  \operatorname{avg}(\omega_i\omega_j)=\frac{\delta_{ij}}{n},
\]
故球面平均值为
\[
  u(x)+\frac{r^2}{2n}\Delta u(x)+\littleo(r^2).
\]
题设说该平均值对所有充分小的 $r$ 都严格等于 $u(x)$，于是
\[
  \frac{r^2}{2n}\Delta u(x)+\littleo(r^2)=0.
\]
除以 $r^2$ 并令 $r\to0$，得到
\[
  \Delta u(x)=0.
