...
\paragraph{练习题 9}
\begin{xsolution}
对极坐标曲线 $\rho=\rho(\theta)$，若切线与径向的夹角为 $\psi$，由参数方程可得
\[
  \tan\psi=\frac{\rho}{\rho'}.
\]
对 $\rho=a\theta$ 有 $\tan\psi_1=\theta$；对 $\rho=a\theta^{-1}$ 有 $\tan\psi_2=-\theta$。通常取正半径时，两曲线相交要求 $a\theta=a/\theta$，故 $\theta=1$。于是
\[
  \psi_1=\frac\pi4,\qquad \psi_2=-\frac\pi4,
\]
两切线夹角为 $\pi/2$，故正交。
\end{xsolution}
...
\]
非奇异点有水平切线时 $ay-x^2=0$，即 $y=x^2/a$。代回原方程，
\[
...
\paragraph{第二组参考题 2}
% CHECK-SOLUTION: 本章题面只引用“例题 5.1.4 中的 Riemann 函数 R(x)”而未给出 R(x) 的定义；按本任务限制不得读取其他章节或猜测定义，因此无法可靠给出逐点不可导证明。
\begin{xsolution}
本题所需的 $R(x)$ 定义不在第6章题面中。由于不同教材中“Riemann 函数”可能指不同函数，而不可导性的证明取决于具体定义，本解答不作猜测。
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 令 $q=p'$。设 $q$ 的互异实根为 $r_1,\ldots,r_m$。由题设这些根都是简单根，故在 $q(x)\ne0$ 时
  \[
