...
\begin{enumerate}[label=(\arabic*)]
  \item 当 $-2\le h\le-1$ 时，有 $-1\le1+h\le0$。

  若 $n=1$，则
  \[
    (1+h)^n=1+h=1+nh.
  \]
  若 $n\ge2$，则 $1+nh\le1-n\le-1$；另一方面，因 $-1\le1+h\le0$，故
  \[
    (1+h)^n\ge-1.
  \]
  因此
  \[
    (1+h)^n\ge-1\ge1+nh.
  \]
  故 Bernoulli 不等式在 $-2\le h\le-1$ 时仍成立。
  ...
  \item 先设 $a_i\ge0$（$i=1,\ldots,n$）。直接展开乘积可得
  \[
    \prod_{i=1}^n(1+a_i)
    =1+\sum_{i=1}^n a_i+\sum_{i<j}a_ia_j+\cdots+\prod_{i=1}^n a_i
    \ge1+\sum_{i=1}^n a_i.
  \]

  再设 $-1<a_i\le0$。令 $x_i=-a_i$，则 $0\le x_i<1$。证明
  \[
    \prod_{i=1}^n(1-x_i)\ge1-\sum_{i=1}^n x_i.
  \]
  对 $n$ 作归纳。$n=1$ 时为等式。若结论对 $n-1$ 成立，记
  \[
    S=\sum_{i=1}^{n-1}x_i\ge0,
  \]
  则因 $1-x_n>0$，
  \[
  \begin{aligned}
  ...
  \end{aligned}
  \]
  因而结论对所有 $n$ 成立。代回 $x_i=-a_i$，得
  \[
    \prod_{i=1}^n(1+a_i)\ge1+\sum_{i=1}^n a_i.
  \]
  由于题设各 $a_i$ 同号，两种情况已穷尽。
\end{enumerate}
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \]
  若后文采用“单调增加”表示严格递增，即结论写成 $f(x_1)<f(x_2)$，则相应否定中的最后一个不等号应改为 $f(x_1)\ge f(x_2)$。这样不依赖术语的严格/非严格约定。

  \item 按非严格约定，“$f$ 是区间 $(a,b)$ 上的单调函数”表示 $f$ 非减或非增。其否定要求这两种情形都失败，即
  \[
  \begin{gathered}
  ...
  \end{gathered}
  \]
  两对点不必相同。若“单调”采用严格约定，则上述两个末端不等号分别改为 $\ge$ 与 $\le$。

  \item 若 $A\subset B$ 表示包含关系，即
  \[
  ...
  \]
  若采用 $\subset$ 专指真包含的约定，则 $A\subset B$ 还要求 $A\ne B$，其否定相应为“存在 $x\in A$ 但 $x\notin B$，或 $A=B$”。
