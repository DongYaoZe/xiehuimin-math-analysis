...
设 $\{F_\alpha\}_{\alpha\in I}$ 是一族闭集。则每个 $F_\alpha^c$ 都是开集。由 De Morgan 法则，
\[
  \left(\bigcap_{\alpha\in I}F_\alpha\right)^c
  =\bigcup_{\alpha\in I}F_\alpha^c.
\]
任意多个开集的并集仍为开集，因此右端是开集，从而 $\bigcap_{\alpha\in I}F_\alpha$ 是闭集。

\textbf{(2) 有限多个闭集的并集是闭集。}
设 $F_1,\ldots,F_m$ 为闭集，则 $F_1^c,\ldots,F_m^c$ 都是开集，且
\[
  \left(\bigcup_{i=1}^mF_i\right)^c
  =\bigcap_{i=1}^mF_i^c.
\]
有限多个开集的交集仍为开集，所以 $\bigcup_{i=1}^mF_i$ 为闭集。
\end{xsolution}
...
反过来，任一收敛点列都是 Cauchy 点列，这是三角形不等式的直接推论。故 Cauchy 收敛准则成立。
\end{xsolution}
...
\]
由于 $x\mapsto d(x,F)$ 是连续函数，每个 $G_m$ 都是开集，而且 $F\subset G_m$，故
\[
...
\]
先确定其闭包。由于 $-1\leq\sin(1/x)\leq1$，任何趋于 $x=0$ 的聚点第二坐标都在 $[-1,1]$ 中。反之，对任意 $y_0\in[-1,1]$，取 $\theta$ 使 $\sin\theta=y_0$，再令
\[
  x_k=\frac1{\theta+2k\pi}
\]
（忽略有限个使分母非正的项），则 $x_k\to0$ 且
\[
  \sin\frac1{x_k}=y_0.
\]
因此
...
另一方面，$K+C$ 是 $\ZZ^2$ 在映射 $(m,n)\mapsto m+\alpha n$ 下的像，因此是可列集；而 $\RR$ 不可列，所以
\[
  K+C\neq\RR.
\]
故两个闭集之和未必是闭集。
