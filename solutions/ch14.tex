...
\]
等号不一定成立。例如令 $a_n$ 只在偶数 $n$ 非零，$b_n$ 只在奇数 $n$ 非零，则 $a_nb_n\equiv0$，乘积幂级数的收敛半径为 $+\infty$。若两个根值极限都存在而不是只有上极限，则通常可直接得到 $R_{ab}=R_1R_2$。当某个半径为 $0$ 或 $+\infty$ 时，应以 $\rho=1/R$ 的上极限形式理解上述结论，避免 $0\cdot\infty$ 的歧义。
\end{xsolution}
...
\begin{xsolution}
可以。设幂级数的收敛半径为 $R>0$，任取 $0<r<R$。再取 $\rho$ 满足 $r<\rho<R$。因为级数在 $x=x_0+\rho$ 收敛，所以数列 $\{a_n\rho^n\}$ 有界，即存在 $M>0$ 使
\[
  \abs{a_n}\rho^n\leqslant M.
\]
当 $\abs{x-x_0}\leqslant r$ 时，
\[
  \abs{a_n(x-x_0)^n}
  \leqslant M\left(\frac r\rho\right)^n.
\]
右边是收敛几何级数，故由 Weierstrass 判别法，幂级数在每个内闭区间 $[x_0-r,x_0+r]$ 上绝对一致收敛。这正是 Abel 第二定理中“内闭一致收敛”的结论。

若还要处理某个恰好属于收敛域的端点并证明到该端点的一侧一致收敛或和函数连续，则单纯的上述优级数估计一般不够，需要 Abel 变换等更精细的论证。
\end{xsolution}

\paragraph{14.3.2 思考题 5}
\begin{xsolution}
\textbf{(1)} 一般不能无条件写成给出的等式。对 $0<r<R$，逐项积分当然成立：
\[
  \int_0^r\left(\sum_{n=0}^{\infty}a_nx^n\right)\dd x
  =\sum_{n=0}^{\infty}\frac{a_n}{n+1}r^{n+1}.
\]
但令 $r\to R-$ 时，左、右两边未必有有限极限。例如取 $a_n=R^{-n}$，则和函数为
\[
  \frac1{1-x/R},
\]
其在 $[0,R)$ 上的广义积分发散，同时右边变成 $R\sum_{n=0}^{\infty}1/(n+1)$，也发散。因此不能把 $r=R$ 的等式视为无条件成立的有限等式。
...
收敛，则答案为肯定。令
\[
  F(r)=\sum_{n=0}^{\infty}\frac{a_n}{n+1}r^{n+1},\qquad 0\leqslant r<R.
\]
对 $r<R$ 有
\[
  F(r)=\int_0^r\left(\sum_{n=0}^{\infty}a_nx^n\right)\dd x.
\]
把 $F$ 看成一个幂级数，其在 $r=R$ 处收敛。由 Abel 第二定理，
\[
  \lim_{r\to R-}F(r)
  =\sum_{n=0}^{\infty}\frac{a_n}{n+1}R^{n+1}.
\]
左边正是和函数在 $[0,R)$ 上广义积分的极限。因此此时
\[
  \int_0^R\left(\sum_{n=0}^{\infty}a_nx^n\right)\dd x
  =\sum_{n=0}^{\infty}\frac{a_n}{n+1}R^{n+1}.
\]
...
\[
  ...
  \qquad x\in(-a,a),\quad n=0,1,2,\ldots.
\]
...
\[
  \abs{f^{(n)}(x)}\leqslant M
\]
对一切 $x\in\RR$ 和 $n\geqslant0$ 成立。与上一题相同，Taylor 余项满足
\[
  \abs{r_n(x)}\leqslant M\frac{\abs{x}^{n+1}}{(n+1)!}\to0,
\]
所以 $f$ 在整个实轴上等于其 Maclaurin 级数：
\[
  f(x)=\sum_{m=0}^{\infty}c_mx^m,
  \qquad c_m=\frac{f^{(m)}(0)}{m!}.
\]

若 $f$ 不恒为 $0$，设 $m$ 是最小的使 $c_m\ne0$ 的整数，则
\[
  f(x)=x^m\bigl(c_m+xg(x)\bigr)
\]
在 $0$ 的某个邻域成立，其中 $g$ 连续。于是可取 $\delta>0$，使 $0<\abs{x}<\delta$ 时
\[
  \abs{xg(x)}<\frac{\abs{c_m}}2,
\]
从而 $c_m+xg(x)\ne0$，即 $f(x)\ne0$。这与存在正数列 $x_n\to0$ 且 $f(x_n)=0$ 矛盾。因此
\[
  \boxed{f(x)\equiv0}.
\]
\end{xsolution}
...
乘以区间长度并求和，得
\[
  U(f,P)-L(f,P)<\frac\varepsilon2+rac\varepsilon2=\varepsilon.
\]
所以 $f$ Riemann 可积。
...
\paragraph{第二组参考题 9}
% CHECK-SOLUTION: 题面声称末个严格不等式对“一切实数 x”成立，但 x=0 时两边均为 0，严格不等式 0<0 不成立。下面证明修正后的结论：第一不等式对所有实数 x 成立；末个不等式对 x\ne0 严格成立，x=0 时取等号。
\begin{xsolution}
令 $t=\abs{x}$。由幂级数展开，
\[
...
其中
\[
  R_n(x)=\frac1{n!}\int_0^x f^{(n+1)}(t)(x-t)^n\,\dd t\geqslant0.
\]

对 $0\leqslant t\leqslant x$，在点 $t$ 对 $f(y)$ 作 Taylor 展开。由于 $f$ 及各阶导数均非负，保留第 $n+1$ 项即可得
\[
  f(y)\geqslant
  \frac{f^{(n+1)}(t)}{(n+1)!}(y-t)^{n+1}.
\]
所以
\[
  f^{(n+1)}(t)
  \leqslant\frac{(n+1)!f(y)}{(y-t)^{n+1}}.
\]
代入余项，得到
\[
\begin{aligned}
  0\leqslant R_n(x)
  &\leqslant(n+1)f(y)
  \int_0^x\frac{(x-t)^n}{(y-t)^{n+1}}\,\dd t.
\end{aligned}
\]
令
\[
  u=\frac{x-t}{y-t}.
\]
则 $0\leqslant u\leqslant x/y$，并可算得
\[
  \int_0^x\frac{(x-t)^n}{(y-t)^{n+1}}\,\dd t
  =\int_0^{x/y}\frac{u^n}{1-u}\,\dd u
  \leqslant\frac1{1-x/y}\frac{(x/y)^{n+1}}{n+1}.
\]
因此
\[
  0\leqslant R_n(x)
  \leqslant\frac{f(y)}{1-x/y}\left(\frac xy\right)^{n+1}\to0.
\]
故对每个 $x\in[0,r)$，
\[
  \boxed{
