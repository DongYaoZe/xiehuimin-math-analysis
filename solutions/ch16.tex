...
\[
  ...
  =(1-2^{1-s})\zeta(s),\qquad s>0,
\]
其中在 $s=1$ 附近按 Abel--Dirichlet 方法理解。所求级数收敛（$\ln n/n$ 从充分大的 $n$ 起单调趋于 0），并且
\[
  \sum_{n=1}^{\infty}(-1)^{n-1}\frac{\ln n}{n}=-\eta'(1).
\]
设 $h=s-1$。利用
\[
  1-2^{-h}=h\ln2-\frac{h^2(\ln2)^2}{2}+\bigO(h^3),
\]
以及
\[
  \zeta(1+h)=\frac1h+\gamma+\bigO(h),
\]
其中 $\gamma$ 为 Euler 常数，得到
\[
  \eta(1+h)=\ln2+h\left(\gamma\ln2-\frac{(\ln2)^2}{2}\right)+\bigO(h^2).
\]
故
\[
  \boxed{\displaystyle
