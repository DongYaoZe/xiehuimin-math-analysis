...
\]
其中 $D$ 在 $xOy$ 平面上由 $xy=1,xy=2,y=3x,y=4x$ 围成。$D$ 有第一、第三象限两个全等分支，且被积函数在变换 $(x,y)\mapsto(-x,-y)$ 下不变，故只算第一象限再乘 $2$。
...
\paragraph{第7题}
\begin{xsolution}
由 $p_i>0$ 且 $\sum_{i=1}^m1/p_i=1$ 可知实际有 $p_i>1$。对 $m$ 作归纳。$m=2$ 时就是 Hölder 不等式。

设结论对 $m-1$ 个因子成立，并令
\[
 \frac1r=\sum_{i=1}^{m-1}\frac1{p_i}=1-\frac1{p_m}.
\]
先对 $U=u_1\cdots u_{m-1}$ 与 $u_m$ 用 Hölder 不等式，再对 $U$ 用归纳假设：
\begin{align*}
 \abs*{\iint_\Omega u_1\cdots u_m\,\dd x\,\dd y}
 &\le\norm{U}_r\norm{u_m}_{p_m}\\
 &\le\norm{u_1}_{p_1}\cdots\norm{u_{m-1}}_{p_{m-1}}
 \norm{u_m}_{p_m}.
\end{align*}
