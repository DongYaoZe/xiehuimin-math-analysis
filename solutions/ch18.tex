...
\[
  \frac{\ln(1+xy)}{xy}\to1.
\]
所以在充分小邻域内存在常数 $C>0$，使 $x\ne0$ 时
\[
  \abs{f(x,y)}\le C\abs{y}.
\]
而 $x=0$ 时 $\abs{f(0,y)}=\abs{y}$。取 $C_1\ge\max\{C,1\}$，则原点附近统一有
\[
...
由
\[
  \lim_{x\to x_0}\psi(x)=0
\]
可取 $\delta_2>0$，使
\[
  0<\abs{x-x_0}<\delta_2
  \Longrightarrow
  \abs{\psi(x)}<\frac\varepsilon2.
\]
题设
\[
  \abs{f(x,y)-\varphi(y)}\le\psi(x)
\]
还说明在所讨论邻域内 $\psi(x)\ge0$。于是当两变量同时充分接近相应极限点时，
\[
  \abs{f(x,y)-A}
  \le
  \abs{f(x,y)-\varphi(y)}+\abs{\varphi(y)-A}
  \le
  \psi(x)+\abs{\varphi(y)-A}
  <\varepsilon.
\]
所以
\[
...
\[
  ...
  <\varepsilon.
\]
补充的定义域条件保证这一矩形去心邻域中的点均在函数的定义域内，因此
\[
  \lim_{(x,y)\to(x_0,y_0)}f(x,y)=A.
\]
两个累次极限分别为
...
\textbf{(1)}
% CHECK-SOLUTION: 扫描原题中的 g_k 公式确实缺少因子 M，且写成 d(x,A_k)-d(x,B_k)，与 A_k 对应正残差、B_k 对应负残差的定义方向相反；按原式一般不能推出 g|_D=f。以下明确给出必要修正后的完整证明，原题公式本身不能被伪证。
\begin{xsolution}
先说明原题公式的矛盾。按题面，
\[
...
再证连续性，事实上 $F$ 仍一致连续。给定 $\varepsilon>0$，取原函数一致连续性对应的 $\delta>0$。若
\[
...
令 $m\to\infty$ 得
\[
  \abs{F(\vect{x})-F(\vect{y})}\le\varepsilon.
\]
所以 $F$ 一致连续。
