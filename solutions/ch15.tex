...
\[
  ...
  \qquad0<x<\pi }.
\]
由于奇延拓在 $x=0$ 有跳跃，正弦级数在 $x=0$ 的和为 $0$；在 $0<x<\pi$ 收敛于原函数。
...
\]
这可由可积且绝对可积函数的连续函数 $L^1$ 逼近得到。再由 Weierstrass 第二逼近定理，存在三角多项式 $P_n$ 使
\[
  \max_{[-\pi,\pi]}\abs{g(x)-P_n(x)}
  <\frac{\varepsilon}{4\pi}.
\]
于是
\begin{align*}
  \int_{-\pi}^{\pi}\abs{f(x)-P_n(x)}\,\dd x
  &\le\int_{-\pi}^{\pi}\abs{f-g}\,\dd x
   +\int_{-\pi}^{\pi}\abs{g-P_n}\,\dd x\\
...
\paragraph{参考题 2.}
\begin{xsolution}
下凸函数在闭区间上连续并绝对连续，其导函数几乎处处存在且单调增加，并且
\[
  f(x)=f(0)+\int_0^xf'(t)\,\dd t.
\]
因此对 $n\ge1$ 可作分部积分：
\[
  \int_0^{2\pi}f(x)\cos nx\,\dd x
  =-\frac1n\int_0^{2\pi}f'(x)\sin nx\,\dd x.
\]
$-f'$ 单调减少。参考题 1(1) 的证明只用到了单调性，故同样适用于这里的几乎处处定义的单调代表，从而
\[
  \int_0^{2\pi}[-f'(x)]\sin nx\,\dd x\ge0.
\]
故
...
\]
也可先用分段线性下凸函数一致逼近 $f$，对逼近函数证明后再取极限，避开不可导点。
\end{xsolution}
...
\[
  \sum_{n=1}^{\infty}\frac{(-1)^{n-1}}n\sin nx
\]
本身就是区间 $(-\pi,\pi)$ 上函数 $x/2$ 的 Fourier 级数。因此 $f$ 的非零频率 Fourier 系数正是给定的 $a_n,b_n$。常数项由原级数中的 $a_0/2$ 决定，所以 $f$ 的全部 Fourier 系数就是
$a_0,a_n,b_n$。故原三角级数确为其和函数 $f$ 的 Fourier 级数，并且在 $\varphi$ 的连续点有 $f'=\varphi$。
\end{xsolution}
...
\]
后一个级数绝对一致收敛，所以 $\varphi$ 连续。由参考题 16，原三角级数处处收敛，并且在 $(-\pi,\pi)$ 内其和函数可导，导函数为 $\varphi$；同时该三角级数是这个和函数的 Fourier 级数。端点处的函数值不影响 Fourier 系数，因此求出开区间内的表达式后，可按连续极限重新定义端点，从而得到题目所要求的连续可微函数。
...
\[
  ...
  =\frac{(-1)^n n}{n^2-1}.
\]
当 $n=1$ 时，$x\cos x/2$ 对 $b_1$ 的贡献为 $-1/4$，而 $\sin x/4$ 的贡献为 $1/4$，故 $b_1=0$。因此它的 Fourier 级数正是题给级数。
\end{xsolution}
