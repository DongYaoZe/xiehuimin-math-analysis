...
\]
并未限制常数项 $a_0$。在这一条件下原命题不成立。例如取 $n=2$，令
\[
...
\[
 C_2(x)=2+\cos2x\ge1>0,
\]
在 $[0,2\pi)$ 中没有零点，更不可能有 $4$ 个零点。

若原题本意是常见的较强条件
\[
 \sum_{k=0}^{n-1}|a_k|<|a_n|,
\]
则在 $x_j=j\pi/n$ 处最高次项为 $a_n(-1)^j$，其绝对值严格大于其余项之和，故 $C_n(x_j)$ 的符号随 $j$ 交替；对相邻的 $2n$ 个小区间应用零点存在定理即可得到至少 $2n$ 个零点。但这不是原书当前印出的条件。
\end{xsolution}
...
若不存在这样的 $x_0$，则 $f(x)\le L$ 对所有 $x$。因为 $f$ 非常值，存在 $x_0$ 使 $f(x_0)<L$。完全类似地可证明 $f$ 取到全局最小值。故至少最大值或最小值之一能够取到。
\end{xsolution}
...
\[
 f(a^+)=f(b^-)=+\infty,
\]
存在 $\delta>0$，使在 $(a,a+\delta)$ 与 $(b-\delta,b)$ 上都有
\[
 f(x)>f(x_0)+1.
\]
可令 $a+\delta<b-\delta$。在闭区间 $[a+\delta,b-\delta]$ 上，$f$ 由最值定理取到最小值 $m$。因为 $x_0$ 可取在这个闭区间中，所以 $m\le f(x_0)$；而两端邻近部分函数值均大于 $f(x_0)+1>m$。因此这个 $m$ 也是整个 $(a,b)$ 上的最小值。
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
 ...
 \item 若要求“对每个满足 $f(x)-g(x)\to0$ 的连续函数 $f$，都能推出 $f$ 一致连续”，则 $g$ 在 $[0,+\infty)$ 上一致连续正是自然的充分必要条件（这里也假定 $g$ 连续）。充分性如下：令 $r=f-g$，则 $r\in C[0,+\infty)$ 且 $r(x)\to0$，由例题 5.4.6，$r$ 一致连续；若 $g$ 也一致连续，则 $f=g+r$ 一致连续。必要性则取特殊情形 $f=g$，此时余项恒为 $0$，若结论对所有这样的 $f$ 都成立，便必须有 $g$ 一致连续。
\end{enumerate}
\end{xsolution}
...
\[
 ...
 y_m=m+\frac1{m^{n-1}}.
\]
则 $|y_m-x_m|=m^{1-n}\to0$。另一方面，由二项式展开，
\[
 y_m^n-x_m^n
 =\left(m+\frac1{m^{n-1}}\right)^n-m^n
 \ge n m^{n-1}\frac1{m^{n-1}}=n.
\]
所以两点距离趋于零而函数值之差始终至少为 $n>0$，与一致连续性的序列判据矛盾。因此 $x^n$ 在 $[0,+\infty)$ 上不一致连续。
\end{xsolution}
...
去掉非负条件后结论一般不成立。例如取 $a=3/4$，定义连续函数
\[
 f(x)=\begin{cases}
  -x,&0\le x\le1/4,\\
  x-1/2,&1/4\le x\le3/4,\\
  1-x,&3/4\le x\le1.
 \end{cases}
\]
...
\[
 f(x+3/4)-f(x)
 =\left(\frac14-x\right)-(-x)=\frac14>0,
\]
故不存在 $f(x)=f(x+3/4)$ 的点。
\end{xsolution}
...
\end{enumerate}
必要性由有限极限的局部有界性立即得到。只证第一条的充分性，第二条完全类似。
...
在 $x\to0^+$ 的情形，把“任意远处”改为“任意靠近 $0$”并取趋于 $0$ 的互不相交小区间即可得到同样结论。
\end{xsolution}
...
\paragraph{第一组参考题 13}
\begin{xsolution}
原题使用无符号的“$\infty$”。若按本题自然的无符号无穷大意义，即
\[
 |f(f(x))|\to+\infty,
\]
则结论应理解为 $|f(x)|\to+\infty$。证明如下。

若 $|f(x)|$ 不趋于 $+\infty$，则存在 $x_n\to+\infty$ 使 $\{f(x_n)\}$ 有界。取收敛子列
\[
 f(x_{n_k})\to y_0\in\RR.
\]
由 $f$ 连续，
\[
 f(f(x_{n_k}))\to f(y_0),
\]
右端为有限数，这与 $|f(f(x))|\to+\infty$ 矛盾。因此 $|f(x)|\to+\infty$。

若把原式中的“$\infty$”强行解释成 $+\infty$，则“$f(x)\to+\infty$”这一结论并不成立：反例 $f(x)=-x$ 满足
\[
 f(f(x))=x\to+\infty,
\]
但 $f(x)\to-\infty$。因此原题的无符号记号必须按无符号无穷大理解，或需另加排除该反例的条件。
\end{xsolution}
...
\[
 c\in(\alpha,\beta).
\]
数列有任意靠后的项低于 $c$，也有任意靠后的项高于 $c$。在一次从 $c$ 下方向 $c$ 上方的越过过程中，取第一次越过的相邻两项。由于相邻项之差趋于 $0$，可由这样的越过选出子列 $x_{n_k}$，使
\[
 x_{n_k}\to c,
 \qquad x_{n_k+1}\to c.
\]
而 $x_{n_k+1}=f(x_{n_k})$，由 $f$ 连续得到
\[
 f(c)=c.
\]
因此区间 $(\alpha,\beta)$ 中每一点都是 $f$ 的不动点。

另一方面，由 $\alpha<\beta$，数列必有充分靠后的项落在 $(\alpha,\beta)$ 中；事实上，若它总只在该开区间两侧，而相邻差又趋于 $0$，就不可能无限次从接近 $\alpha$ 的区域走到接近 $\beta$ 的区域。设某个 $x_N\in(\alpha,\beta)$，则
\[
 x_{N+1}=f(x_N)=x_N,
\]
以后数列恒等于 $x_N$，这与 $\alpha<\beta$ 矛盾。因此只能有 $\alpha=\beta$，故 $\{x_n\}$ 收敛。
\end{xsolution}
