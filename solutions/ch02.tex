...
\]
与 $as_n$ 相减，或直接使用几何级数求导的有限形式。因 $\abs a<1$，极限等于无穷和
\[
  \sum_{k=1}^\infty(2k-1)a^k
  =2\sum_{k=1}^\infty ka^k-\sum_{k=1}^\infty a^k.
\]
利用
\[
...
把奇偶项分别处理。对偶数项，
\[
  x_{2m}=x_0+\sum_{k=1}^m(x_{2k}-x_{2k-2}).
\]
因括号中的数列趋于 $0$，由 Cauchy 命题
\[
  \frac{x_{2m}-x_0}{m}\to0,
\]
所以 $x_{2m}/(2m)\to0$。奇数项完全相同，故
\[
  \boxed{\frac{x_n}{n}\to0}.
\]
\end{xsolution}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \[
    ...
    =\left[\left(1+\frac1{n/2}\right)^{n/2}\right]^2\to\ee^2.
  \]
  \item
  \[
...
\paragraph{第一组参考题 17}
\begin{xsolution}
由 $(1-x_n)x_{n+1}\ge1/4$ 且 $x_{n+1}>0$ 可知 $x_n<1$。同时
\[
...
\[
  u_n-u_{n+1}=u_{n+1}^2.
\]
又已知部分和 $u_1+\cdots+u_n$ 有有限极限，所以非负级数 $\sum u_n$ 收敛，特别有 $u_n\to0$。

若不是所有 $u_n$ 都为 $0$，则从某项起 $u_n>0$。此时
\[
...
\[
  \frac{1/u_n}{n}\to1,
\]
即 $nu_n\to1$。于是充分大的 $n$ 有 $u_n>1/(2n)$，这与 $\sum u_n$ 收敛矛盾。因此只能有
\[
  \boxed{u_n=0\quad\text{对所有 }n}.
\]
\end{xsolution}
...
\]
则在 $n/4\le k\le3n/4$ 的约 $n/2$ 个指标上，
\[
  \frac1{\sqrt{k(n+1-k)}}\ge\frac2{n+1},
\]
故卷积和始终有一个趋近于 $1$ 的正下界，不能趋于 $0$。
\end{xsolution}
...
逆命题不成立。例如 $x_n=1+(-1)^n/2$ 对所有 $n$ 都为正，但数列不收敛，更谈不上收敛到正数 $a$。
\end{xsolution}
