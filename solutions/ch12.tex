...
\textbf{(3)} 在 $x=0$ 及 $x=1$ 邻近，局部可积的共同条件为 $p>-1$。无穷远处
\[
  \frac{|\ln x|^p}{x}\to0
\]
且最终单调减少，因此带 $\sin x$ 的积分由 Dirichlet 判别法收敛。绝对值积分在无穷远处要与
$\int (\ln x)^p\,\dd x/x$ 比较，它收敛需 $p<-1$，与局部条件不相容。故
\[
...
\paragraph{9.}
\textbf{(1)} 因 $p,q,r>0$，$0$ 点有
\[
  \frac{x^p}{1+x^q}|\sin x|^r\sim x^{p+r},
\]
总可积。无穷远处与 $x^{p-q}|\sin x|^r$ 同型。函数 $|\sin x|^r$ 的周期平均值严格为正，因此积分收敛当且仅当
\[
  p-q<-1,
\]
即
\[
  \boxed{q>p+1\text{ 时绝对收敛； }q\leq p+1\text{ 时发散}.}
\]
被积函数非负，无条件收敛。
