...
\[
  ...
  \norm{\vect{x}}=1
\]
的两组相反单位解。该写法也包括某些 $p,q,r$ 为零、$\lambda_\pm$ 恰与 $A,B,C$ 中某个数相等的退化情形。
\end{xsolution}
...
\[
  s=\frac{a+b+c+d}{2},
\]
并记 $\angle A=A,\angle C=C$，面积为 $K$。沿对角线 $BD$ 分成两个三角形。由余弦定理，
\[
...
\begin{aligned}
  4K^2
  ={}&(ab\sin A+cd\sin C)^2\\
  ={}&(ab)^2+(cd)^2-2abcd\cos(A+C)\\
  &-\left(\frac{a^2+b^2-c^2-d^2}{2}\right)^2.
\end{aligned}
\]
利用
\[
  \cos(A+C)=2\cos^2\frac{A+C}{2}-1,
\]
上式化为
\[
\begin{aligned}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 由 (1)，曲面 $z=z(x,y)$ 在任一点 $(x,y,z)$ 的切平面
  \[
    Z-z=z_x(X-x)+z_y(Y-y)
  \]
  经过 $(a,b,c)$。代入该定点得
  ...
  \]
  因而非零向量 $(x-a,y-b)^{\mathsf T}$ 是 Hesse 矩阵
  \[
    \begin{pmatrix}z_{xx}&z_{xy}\\z_{xy}&z_{yy}\end{pmatrix}
  \]
  的零特征向量，于是该矩阵行列式为零：
  \[
