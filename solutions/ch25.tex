...
% CHECK-SOLUTION: 本题还要求“利用这个结果重新计算例题21.4.2”，但任务明确限定只读本章 chapters/ch25.tex；本章没有给出例题21.4.2的题面，无法在不猜测题意的前提下可靠复算。上面的投影面积结论已完整证明。
\end{xsolution}
...
\[
  ...
  z(\cos^2\theta+\sin^2\theta)\,\dd z\,\dd\theta=0.
\]
\end{xsolution}
...
\paragraph{参考题 7}
\begin{xsolution}
取一个以 $\Gamma$ 为边界、定向与 $\Gamma$ 相容的逐片光滑曲面 $\Sigma$。为得到单值势函数，先把 $\Sigma$ 作为割面；在 $\RR^3\setminus\Sigma$ 的每个连通分支上定义
\[
\boxed{
  u(\vect x)=
  \iint_\Sigma
  \frac{(\vect x-\vect s)\cdot\vect n(\vect s)}{|\vect x-\vect s|^3}\,\dd S_{\vect s}+C}.
\]
这就是有向曲线 $\Gamma$ 在点 $\vect x$ 所张的有向立体角（差一个加法常数）。

下面验证它的梯度。对每个分量应用二维 Stokes 公式可得标准边界恒等式
\[
  \nabla_{\vect x}u
  =\oint_\Gamma\frac{(\vect x-\vect s)\times\dd\vect s}{|\vect x-\vect s|^3}.
\]
而
\[
  (\vect x-\vect s)\times\dd\vect s
  =\dd\vect s\times(\vect s-\vect x),
\]
故右端的三个分量恰为题中 $P,Q,R$。因此
\[
  \boxed{\dd u=P\,\dd x+Q\,\dd y+R\,\dd z}.
\]

需要说明的是，$\RR^3\setminus\Gamma$ 一般不是单连通区域，因而通常不存在在整个补集上单值的全局势函数；上述 $u$ 在跨过所选割面 $\Sigma$ 时相差 $4\pi$ 的整数倍。这并不影响其在每个割开后的连通区域内作为题中微分式的势函数。
\end{xsolution}
